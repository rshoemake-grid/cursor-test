"""Seed script to add the File Processing template to the marketplace"""
import asyncio
import sys
import os
...
async def seed_file_processing_template():
    """Create the File Processing template in the database"""
        ...
        try:
            ...
            result = await session.execute(
                select(WorkflowTemplateDB).where(
                    WorkflowTemplateDB.name == "File Processing Workflow"
                )
            )
            existing = result.scalar_one_or_none()
            
            if existing:
                print("Template 'File Processing Workflow' already exists. Skipping.")
                return
            ...
            template = WorkflowTemplateDB(
                ...
                category="automation",
                tags=["gcp", "file-processing", "data-generation", "automation"],
