...
def get_node_input_config(node: Any) -> Dict[str, Any]:
    ...
    Args:
        node: Node object (or raw dict) with optional data and input_config
    ...
    if isinstance(node, dict):
        ...
        if isinstance(data, dict):
            data_input_config = data.get("input_config")
            if isinstance(data_input_config, dict):
                input_config = data_input_config.copy()
        top_input_config = node.get("input_config")
        if isinstance(top_input_config, dict):
    ...
    if hasattr(node, "input_config") and isinstance(node.input_config, dict):
        top_input_config = node.input_config
        for key, value in top_input_config.items():
            if value and (not isinstance(value, str) or value.strip()):
                input_config[key] = value
            elif key not in input_config:
