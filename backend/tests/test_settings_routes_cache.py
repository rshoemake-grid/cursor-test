...
async def test_get_llm_settings_from_cache(db_session: AsyncSession, test_user: UserDB):
    ...
    _settings_cache[test_user.id] = LLMSettings(
    ...
    try:
        ...
        # Clean up cache
        if test_user.id in _settings_cache:
            del _settings_cache[test_user.id]
...
async def test_load_settings_into_cache(db_session: AsyncSession, test_user: UserDB):
    ...
    from main import app
    from backend.api.settings_routes import load_settings_into_cache, _settings_cache
    ...
    try:
        ...
        await load_settings_into_cache(db_session)
        
        # Check cache was populated
        assert test_user.id in _settings_cache
        cached_settings = _settings_cache[test_user.id]
