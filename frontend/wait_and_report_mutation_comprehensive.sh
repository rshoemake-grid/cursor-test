...
# Comprehensive Wait and Report Script
# Combines features from wait_and_report_mutation.sh and wait-and-report-mutation.sh
# Waits for mutation testing to complete and reports results
...
LOG_FILE="${MUTATION_LOG_FILE:-mutation_output.log}"
if [ ! -f "$LOG_FILE" ]; then
...
report_results() {
    ...
    elif [ -f "reports/mutation/mutation.html" ]; then
        echo "HTML Report: reports/mutation/mutation.html"
        ls -lh reports/mutation/mutation.html 2>/dev/null || echo "HTML report not found"
    else
...
while true; do
    ...
    if is_running; then
        ...
        if [ -f "$LOG_FILE" ]; then
            PROGRESS=$(tail -50 "$LOG_FILE" | grep -E "(Mutant|progress|Tested|Mutation testing.*%)" | tail -1)
            if [ -n "$PROGRESS" ]; then
