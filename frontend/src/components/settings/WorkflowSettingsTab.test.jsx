...
describe("WorkflowSettingsTab", () => {
  ...
  it("should format model options with provider name", () => {
    render(<WorkflowSettingsTab {...defaultProps} />);
    const select = screen.getByLabelText("Default Model");
    const gpt4Option = Array.from(select.options).find(
      (opt) => opt.value === "gpt-4",
    );
  ...
  it("should render help text for default model", () => {
    render(<WorkflowSettingsTab {...defaultProps} />);
    expect(
      screen.getByText(/Default model for workflow execution and agent nodes/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Only models from enabled providers are shown/),
    ).toBeInTheDocument();
  });
});
