...
describe("ProviderForm", () => {
  ...
  it("should call onToggleProviderModels when expand button is clicked", () => {
    ...
    expect(defaultProps.onToggleProviderModels).toHaveBeenCalledWith(
      "provider-1",
    );
  });
  ...
  describe("Expanded form fields", () => {
    ...
    it("should toggle API key visibility", () => {
      ...
      expect(defaultProps.onToggleApiKeyVisibility).toHaveBeenCalledWith(
        "provider-1",
      );
    });
    ...
    it("should show loading state when testing", () => {
      render(<ProviderForm {...expandedProps} testingProvider="provider-1" />);
  ...
  describe("Model list interactions", () => {
    const expandedProps = {
      ...
      expandedModels: {
        "provider-1": new Set(["gpt-4"]),
      },
      isModelExpanded: jest.fn(),
    };
    beforeEach(() => {
      expandedProps.isModelExpanded.mockImplementation(
    ...
    it("should expand model when clicked", () => {
      ...
      const modelButtons = screen
        ...
        .filter(Boolean);
      const modelListButton = modelButtons.find(
        (btn) => btn?.textContent?.includes("gpt-4") && !btn?.closest("select"),
      );
      if (modelListButton) {
        fireEvent.click(modelListButton);
        expect(defaultProps.onToggleModel).toHaveBeenCalledWith(
          "provider-1",
          "gpt-4",
        );
      } else {
    ...
    it("should show model edit form when expanded", () => {
      ...
      const modelNameInput = inputs.find(
        (input) =>
    ...
    it("should update model name when changed", () => {
      ...
      const inputs = screen.getAllByRole("textbox");
      const modelNameInput = inputs.find(
        (input) => input.getAttribute("value") === "gpt-4",
      );
      expect(modelNameInput).toBeInTheDocument();
      if (modelNameInput) {
        ...
        expect(defaultProps.onUpdateProvider).toHaveBeenCalledWith(
          "provider-1",
          {
    ...
    it("should set model as default when button clicked", () => {
      const propsWithNonDefaultExpanded = {
        ...expandedProps,
        expandedModels: {
          "provider-1": new Set(["gpt-3.5-turbo"]),
        },
    ...
    it('should show "Default Model" button when model is default', () => {
      ...
      const buttons = screen.getAllByRole("button");
      const defaultModelButton = buttons.find(
        (btn) => btn.textContent === "Default Model",
      );
