  ...
  describe("Model list interactions", () => {
    ...
    it("should show model edit form when expanded", () => {
      ...
      const modelNameInput = inputs.find(
        (input) => input.getAttribute("value") === "gpt-4" || input.closest("div")?.querySelector("label")?.textContent === "Model Name"
      );
    ...
    it("should set model as default when button clicked", () => {
      const propsWithNonDefaultExpanded = {
        ...expandedProps,
        expandedModels: { "provider-1": /* @__PURE__ */ new Set(["gpt-3.5-turbo"]) },
        isModelExpanded: jest.fn((providerId, modelName) => {
          return providerId === "provider-1" && modelName === "gpt-3.5-turbo";
        })
      };
      render(/* @__PURE__ */ jsx(ProviderForm, { ...propsWithNonDefaultExpanded }));
