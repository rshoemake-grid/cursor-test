...
describe("WorkflowList", () => {
  ...
  it("should handle error when loading workflows", async () => {
    ...
    await waitForWithTimeout(() => {
      expect(showError).toHaveBeenCalledWith(
        "Failed to load workflows: Failed to load",
      );
    });
  });
  ...
  it("should handle deletion error", async () => {
    ...
    await waitForWithTimeout(() => {
      ...
      expect(showError).toHaveBeenCalledWith(
        "Failed to delete workflow: Delete failed",
      );
    });
  });
  it("should toggle workflow selection", async () => {
    ...
    const selectButtons = screen.getAllByTitle(
      /Select workflow|Deselect workflow/,
    );
  ...
  it("should handle bulk duplicate", async () => {
    ...
    mockApi.getWorkflows.mockResolvedValueOnce([
      ...mockWorkflows,
      {
        ...mockWorkflows[0],
        id: "workflow-1-copy",
  ...
  it("should show login prompt when not authenticated", async () => {
    ...
    await waitForWithTimeout(() => {
      ...
      expect(
        screen.getByText(/Browse templates on the Marketplace/),
      ).toBeInTheDocument();
      expect(
  ...
  it("should handle bulk delete with partial failures", async () => {
    ...
    await waitForWithTimeout(() => {
      expect(showError).toHaveBeenCalledWith(
        expect.stringContaining("Failed IDs: workflow-2"),
      );
    });
  });
  it("should handle bulk duplicate with partial failures", async () => {
    ...
    await waitForWithTimeout(() => {
      expect(showError).toHaveBeenCalledWith(
        expect.stringContaining("Failed to duplicate workflow"),
      );
    });
  });
  ...
  it("should handle bulk delete error", async () => {
    mockApi.getWorkflows.mockResolvedValue(mockWorkflows);
    mockApi.bulkDeleteWorkflows.mockRejectedValue(
    ...
    await waitForWithTimeout(() => {
      expect(showError).toHaveBeenCalledWith(
        "Failed to delete workflows: Bulk delete failed",
      );
    });
  });
  ...
  describe("Publish functionality", () => {
    it("should open publish modal when publish button is clicked", async () => {
      mockApi.getWorkflows.mockResolvedValue(mockWorkflows);
      renderWithRouter(
        <WorkflowList onSelectWorkflow={mockOnSelectWorkflow} />,
      );
      await waitForWithTimeout(() => {
        expect(screen.getByText("Test Workflow 1")).toBeInTheDocument();
      });
      const publishButtons = screen
        ...
        .filter(
          (btn) =>
            ...
            btn.getAttribute("title")?.includes("Publish"),
        );
      if (publishButtons.length > 0) {
        fireEvent.click(publishButtons[0]);
        await waitForWithTimeout(() => {
          expect(
            screen.getByText(/Publish to Marketplace/),
          ).toBeInTheDocument();
        });
      }
    });
