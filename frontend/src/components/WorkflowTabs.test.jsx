...
describe('WorkflowTabs', ()=>{
    ...
    const renderWithProvider = (props = {})=>{
        ...
        return render(/*#__PURE__*/ _jsx(WorkflowTabsProvider, {
            initialTabs: initialTabs || [],
            initialActiveTabId: initialActiveTabId !== undefined ? initialActiveTabId : null,
            storage: storage,
            children: /*#__PURE__*/ _jsx(WorkflowTabs, {
    ...
    it('should render with default tab', ()=>{
        ...
        const tabButtons = screen.getAllByRole('button').filter((btn)=>btn.textContent?.includes('Untitled Workflow'));
        expect(tabButtons.length).toBeGreaterThan(0);
    });
    it('should create new tab when plus button is clicked', async ()=>{
        renderWithProvider();
        const initialTabCount = screen.getAllByRole('button').filter((btn)=>btn.textContent?.includes('Untitled Workflow')).length;
    ...
    it('should start editing tab name on double click', async ()=>{
        ...
        const tabButtons = screen.getAllByRole('button').filter((btn)=>btn.textContent?.includes('Untitled Workflow') && btn.getAttribute('title') !== 'Close tab');
    ...
    it('should cancel editing on Escape', async ()=>{
        ...
        if (tabButtons.length > 0) {
            ...
            await waitForWithTimeout(()=>{
                const tabButtonsAfter = screen.getAllByRole('button').filter((btn)=>btn.textContent?.includes('Untitled Workflow'));
    ...
    it('should load workflow when initialWorkflowId is provided', async ()=>{
        ...
        await waitForWithTimeout(()=>{
            ...
            const tabButtons = screen.getAllByRole('button').filter((btn)=>btn.textContent?.includes('Loading') || btn.textContent?.includes('Loaded Workflow') || btn.textContent?.includes('Untitled Workflow'));
            expect(tabButtons.length).toBeGreaterThan(0);
        }, 3000);
    ...
    it('should handle tab rename error', async ()=>{
        ...
        const tabButtons = screen.getAllByRole('button').filter((btn)=>btn.textContent?.includes('Test Workflow'));
        if (tabButtons.length > 0) {
    ...
    describe('Initial workflow loading edge cases', ()=>{
        it('should handle initialWorkflowId with same workflowLoadKey twice', async ()=>{
            ...
            rerender(/*#__PURE__*/ _jsx(WorkflowTabsProvider, {
                initialTabs: [],
