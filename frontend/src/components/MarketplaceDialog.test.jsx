...
describe('MarketplaceDialog', ()=>{
    ...
    it('should not render when isOpen is false', ()=>{
        render(/*#__PURE__*/ _jsx(MarketplaceDialog, {
            isOpen: false,
            onClose: mockOnClose
        }));
    ...
    it('should handle category selection', ()=>{
        ...
        const selects = screen.getAllByRole('combobox');
        const categorySelect = selects.find((select)=>select.closest('div')?.textContent?.includes('Category'));
        if (categorySelect) {
    ...
    it('should handle difficulty selection', ()=>{
        ...
        const selects = screen.getAllByRole('combobox');
        const difficultySelect = selects.find((select)=>select.closest('div')?.textContent?.includes('Difficulty'));
        if (difficultySelect) {
    ...
    it('should update form when node changes', ()=>{
        const { rerender } = render(/*#__PURE__*/ _jsx(MarketplaceDialog, {
            isOpen: true,
            onClose: mockOnClose,
            node: mockNode
        }));
        ...
        rerender(/*#__PURE__*/ _jsx(MarketplaceDialog, {
            isOpen: true,
            onClose: mockOnClose,
            node: newNode
        }));
    ...
    describe('Edge cases', ()=>{
        ...
        it('should handle workflow publishing with empty estimated_time', async ()=>{
            ...
            await waitForWithTimeout(()=>{
                expect(mockApi.publishWorkflow).toHaveBeenCalledWith('workflow-1', expect.objectContaining({
