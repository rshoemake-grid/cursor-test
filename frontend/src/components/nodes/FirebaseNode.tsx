...
export default function FirebaseNode({ data, selected }: NodeProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="font-semibold text-sm text-gray-900 truncate flex-1 min-w-0">{String(data.label || 'Firebase')}</div>
      </div>
      ...
      {inputConfig.firebase_service && (
        <div className="text-xs text-gray-600 bg-gray-50 px-2 py-1 rounded mb-1 truncate">
          Service: {inputConfig.firebase_service}
        </div>
      )}
