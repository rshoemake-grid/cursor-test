...
export default function BigQueryNode({ data, selected }: NodeProps) {
    ...
    <div
      className={`px-4 py-3 shadow-lg rounded-lg bg-white border-2 min-w-[200px] max-w-[200px] ${
        hasError 
          ...
          : selected 
            ? 'border-blue-500 border-4 shadow-xl ring-2 ring-blue-200' 
            : 'border-blue-300'
      }`}
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="font-semibold text-sm text-gray-900 truncate flex-1 min-w-0">{String(data.label || 'BigQuery')}</div>
      </div>
