import { API_CONFIG, STORAGE_KEYS, DEFAULT_VALUES, UI_CONSTANTS, NODE_TYPES, buildStorageKey, getChatHistoryKey } from './constants';
describe('constants', ()=>{
    describe('API_CONFIG', ()=>{
    ...
    describe('buildStorageKey edge cases', ()=>{
        ...
        it('should handle null suffix', ()=>{
            expect(buildStorageKey('prefix', null)).toBe('prefix');
        });
    ...
    describe('API_BASE_URL environment variable handling', ()=>{
        ...
        it('should have valid URL format (absolute or same-origin path for CRA proxy)', ()=>{
            const base = API_CONFIG.BASE_URL;
            expect(base.startsWith('/') || /^https?:\/\//.test(base)).toBe(true);
        });
    });
});
