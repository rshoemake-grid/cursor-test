  ...
  describe('additional coverage for no-coverage mutants', () => {
    describe('fetchWorkflowsOfWorkflows - inner catch block', () => {
      it('should handle error when checking individual workflow', async () => {
        ...
        mockHttpClient.post
          ...
          .mockRejectedValueOnce(new Error('Failed to fetch workflow'))
        ...
        // Should log error for failed workflow but continue processing
        expect(mockLoggerError).toHaveBeenCalledWith(
      ...
      it('should handle workflowResponse.ok === false', async () => {
        ...
        // Should not add workflow when response.ok is false
        expect(result.current.workflowsOfWorkflows).toHaveLength(0)
      })
      ...
      it('should handle workflowDetail.nodes is not an array', async () => {
        ...
        // Should not add workflow when nodes is not an array
        expect(result.current.workflowsOfWorkflows).toHaveLength(0)
      })
    })
    ...
    describe('fetchAgents - migration and edge cases', () => {
      it('should handle migration when user exists but agentsData.length === 0', async () => {
        ...
        // Should not call storage.setItem when agentsData.length === 0
        expect(mockStorage.setItem).not.toHaveBeenCalled()
      })
      ...
      it('should handle migration when user exists but all agents have author_id', async () => {
        ...
        // Should not call storage.setItem when updated is false
        expect(mockStorage.setItem).not.toHaveBeenCalled()
      })
      ...
      it('should handle migration when updated is true but storage is null', async () => {
        ...
        // Should not call storage.setItem when storage is null
        // (Note: mockStorage.setItem won't be called since storage is null)
        expect(result.current.agents.length).toBeGreaterThan(0)
      })
      ...
      it('should handle user with email but no username', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            searchQuery: '',
            sortBy: 'popular',
            user: { id: 'user-1', email: 'test@example.com' }, // No username
            activeTab: 'agents',
            repositorySubTab: 'workflows',
          })
        )
        ...
        // Should use email as author_name
      ...
      it('should handle user with neither username nor email', async () => {
        ...
        // Should use null as author_name
    ...
    describe('fetchRepositoryAgents - error handling', () => {
      it('should handle storage is null', async () => {
        ...
        // Should return early with empty array
        expect(result.current.repositoryAgents).toHaveLength(0)
      })
      ...
      it('should handle JSON.parse error in fetchRepositoryAgents', async () => {
        ...
        // Should log error and use empty array
        expect(mockLoggerError).toHaveBeenCalledWith(
          'Failed to load repository agents from storage:',
          expect.any(Error)
        )
  ...
  describe('mutation killers - conditional expressions', () => {
      ...
      it('should verify exact if (category) check - empty string', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            httpClient: mockHttpClient,
            apiBaseUrl: 'http://api.test',
            category: '', // Empty string - should not append
            searchQuery: '',
            sortBy: 'popular',
            user: null,
            activeTab: 'agents',
      ...
      it('should verify exact if (category) check - truthy value', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            httpClient: mockHttpClient,
            apiBaseUrl: 'http://api.test',
            category: 'automation', // Truthy - should append
            searchQuery: '',
            sortBy: 'popular',
            user: null,
            activeTab: 'agents',
      ...
      it('should verify exact if (searchQuery) check - empty string', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            apiBaseUrl: 'http://api.test',
            category: '',
            searchQuery: '', // Empty string - should not append
            sortBy: 'popular',
            user: null,
            activeTab: 'agents',
            repositorySubTab: 'workflows',
      ...
      it('should verify exact if (searchQuery) check - truthy value', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            apiBaseUrl: 'http://api.test',
            category: '',
            searchQuery: 'test', // Truthy - should append
            sortBy: 'popular',
            user: null,
            activeTab: 'agents',
            repositorySubTab: 'workflows',
    ...
    describe('fetchWorkflowsOfWorkflows - complex logical OR chains', () => {
      ...
      it('should verify hasWorkflowId branch (first OR condition)', async () => {
        ...
        mockHttpClient.post.mockResolvedValue({
          ...
          json: async () => ({
            nodes: [{
              workflow_id: 'workflow-123', // First condition true
              data: {},
            }],
          }),
        })
      ...
      it('should verify nodeData.workflow_id branch (second OR condition)', async () => {
        ...
        mockHttpClient.post.mockResolvedValue({
          ...
          json: async () => ({
            nodes: [{
              data: { workflow_id: 'workflow-123' }, // Second condition true
            }],
          }),
        })
      ...
      it('should verify description.includes("workflow") branch (third OR condition)', async () => {
        ...
        mockHttpClient.post.mockResolvedValue({
          ...
          json: async () => ({
            nodes: [{
              description: 'This is a workflow node', // Third condition true
              data: {},
            }],
          }),
        })
      ...
      it('should verify name.includes("workflow") branch (fourth OR condition)', async () => {
        ...
        mockHttpClient.post.mockResolvedValue({
          ...
          json: async () => ({
            nodes: [{
              name: 'Workflow Node', // Fourth condition true
              data: {},
            }],
          }),
        })
      ...
      it('should verify workflow.tags && workflow.tags.some exact check', async () => {
        const template = { ...mockTemplate, id: 'template-1', tags: null } // tags is null
        mockHttpClient.get.mockResolvedValue({
          json: async () => [template],
        })
        mockHttpClient.post.mockResolvedValue({
    ...
    describe('fetchAgents - sorting logic mutations', () => {
      ...
      it('should verify published_at ? new Date().getTime() : 0 - undefined case', async () => {
        const agent1 = { ...mockAgent, id: 'agent-1', published_at: undefined, is_official: false }
        const agent2 = { ...mockAgent, id: 'agent-2', published_at: '2024-01-01T00:00:00Z', is_official: false }
        mockGetLocalStorageItem.mockReturnValue([agent1, agent2])
      ...
      it('should verify a.name || "" fallback in localeCompare', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            category: '',
            searchQuery: '',
            sortBy: 'alphabetical', // Not 'popular' or 'recent'
            user: null,
            activeTab: 'agents',
            repositorySubTab: 'workflows',
          })
      ...
      it('should verify search filter - name.includes || description.includes || tags.some', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            apiBaseUrl: 'http://api.test',
            category: '',
            searchQuery: 'test', // Should match all three
            sortBy: 'popular',
            user: null,
            activeTab: 'agents',
            repositorySubTab: 'workflows',
      ...
      it('should verify a.name.toLowerCase().includes(query) exact check', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            apiBaseUrl: 'http://api.test',
            category: '',
            searchQuery: 'test', // Lowercase query
            sortBy: 'popular',
            user: null,
            activeTab: 'agents',
            repositorySubTab: 'workflows',
      ...
      it('should verify activeTab === repository exact comparison', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            sortBy: 'popular',
            user: null,
            activeTab: 'repository', // Exact match
            repositorySubTab: 'workflows',
          })
        )
      ...
      it('should verify repositorySubTab === workflows exact comparison', async () => {
        ...
        const { result } = renderHook(() =>
          useMarketplaceData({
            ...
            user: null,
            activeTab: 'repository',
            repositorySubTab: 'workflows', // Exact match
          })
        )
        ...
        // Should call fetchTemplates (not fetchRepositoryAgents)
        expect(mockHttpClient.get).toHaveBeenCalled()
