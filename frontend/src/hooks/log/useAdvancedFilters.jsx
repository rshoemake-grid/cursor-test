  ...
  const filteredExecutions = useMemo(() => {
    ...
    if (filters.minDuration !== void 0 || filters.maxDuration !== void 0) {
      result = result.filter((execution) => {
        ...
        ) : Math.floor((Date.now() - new Date(execution.started_at).getTime()) / 1e3);
        const minMatch = filters.minDuration === void 0 || duration >= filters.minDuration;
        const maxMatch = filters.maxDuration === void 0 || duration <= filters.maxDuration;
    ...
    if (filters.workflowIds && filters.workflowIds.length > 0) {
      result = result.filter(
        (execution) => filters.workflowIds.includes(execution.workflow_id)
      );
    }
