...
describe('useAuthenticatedApi', ()=>{
    ...
    it('should include Authorization header when token is present', ()=>{
        ...
        result.current.authenticatedPost('/test', {
            data: 'test'
        });
        expect(mockPost).toHaveBeenCalledWith(`${API_CONFIG.BASE_URL}/test`, {
            data: 'test'
    ...
    it('should not include Authorization header when token is missing', ()=>{
        ...
        expect(mockPost).toHaveBeenCalledWith(expect.any(String), expect.any(Object), expect.not.objectContaining({
            Authorization: expect.any(String)
        }));
    });
    it('should use custom apiBaseUrl when provided', ()=>{
        const customUrl = 'https://custom-api.com/api';
        const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient, customUrl));
        result.current.authenticatedGet('/test');
        expect(mockGet).toHaveBeenCalledWith(`${customUrl}/test`, expect.objectContaining({
            Authorization: 'Bearer test-token'
        }));
    });
    it('should merge additional headers', ()=>{
        ...
        expect(mockPost).toHaveBeenCalledWith(expect.any(String), expect.any(Object), expect.objectContaining({
            'Content-Type': 'application/json',
            Authorization: 'Bearer test-token',
            'X-Custom-Header': 'value'
        }));
    ...
    it('should handle GET requests', ()=>{
        const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient));
        result.current.authenticatedGet('/test');
        expect(mockGet).toHaveBeenCalledWith(`${API_CONFIG.BASE_URL}/test`, expect.objectContaining({
            Authorization: 'Bearer test-token'
        }));
    });
    it('should handle PUT requests', ()=>{
        const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient));
        result.current.authenticatedPut('/test', {
            data: 'test'
        });
        expect(mockPut).toHaveBeenCalledWith(`${API_CONFIG.BASE_URL}/test`, {
            data: 'test'
        }, expect.objectContaining({
            'Content-Type': 'application/json',
            Authorization: 'Bearer test-token'
    ...
    it('should handle DELETE requests', ()=>{
        ...
        expect(mockDelete).toHaveBeenCalledWith(`${API_CONFIG.BASE_URL}/test`, expect.objectContaining({
            Authorization: 'Bearer test-token'
        }));
    });
    it('should handle GET requests without token', ()=>{
        ...
        const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient));
        result.current.authenticatedGet('/test');
        expect(mockGet).toHaveBeenCalledWith(`${API_CONFIG.BASE_URL}/test`, expect.not.objectContaining({
            Authorization: expect.any(String)
        }));
    });
    it('should handle PUT requests without token', ()=>{
        ...
        expect(mockPut).toHaveBeenCalledWith(expect.any(String), expect.any(Object), expect.not.objectContaining({
            Authorization: expect.any(String)
        }));
    });
    it('should handle DELETE requests without token', ()=>{
        ...
        const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient));
        result.current.authenticatedDelete('/test');
        expect(mockDelete).toHaveBeenCalledWith(`${API_CONFIG.BASE_URL}/test`, expect.not.objectContaining({
            Authorization: expect.any(String)
        }));
    });
    ...
    it('should throw error when client is not properly initialized', async ()=>{
        ...
        await expect(result.current.authenticatedPost('/test', {
            data: 'test'
    ...
    describe('edge cases for 100% mutation coverage', ()=>{
        ...
        it('should verify apiBaseUrl || API_CONFIG.BASE_URL - apiBaseUrl is undefined', ()=>{
            ...
            expect(mockGet).toHaveBeenCalledWith(`${API_CONFIG.BASE_URL}/test`, expect.any(Object));
        });
        ...
        it('should verify !url || url.trim() === "" - url is empty string after construction', async ()=>{
            ...
            await expect(result.current.authenticatedPost('', {
                data: 'test'
        ...
        it('should verify template literal ${baseUrl}${endpoint} - both values', ()=>{
            ...
            expect(mockGet).toHaveBeenCalledWith('https://custom-api.com/api/endpoint', expect.any(Object));
        });
        ...
        it('should verify !client || typeof client.get !== function for GET', async ()=>{
            ...
            const { result } = renderHook(()=>useAuthenticatedApi(invalidClient));
            await expect(result.current.authenticatedGet('/test')).rejects.toThrow(HTTP_CLIENT_ERROR_MSG);
        });
        it('should verify !client || typeof client.put !== function for PUT', async ()=>{
            ...
            const { result } = renderHook(()=>useAuthenticatedApi(invalidClient));
            await expect(result.current.authenticatedPut('/test', {
                data: 'test'
            })).rejects.toThrow(HTTP_CLIENT_ERROR_MSG);
        });
        it('should verify !client || typeof client.delete !== function for DELETE', async ()=>{
            ...
            const { result } = renderHook(()=>useAuthenticatedApi(invalidClient));
            await expect(result.current.authenticatedDelete('/test')).rejects.toThrow(HTTP_CLIENT_ERROR_MSG);
        });
        it('should verify error.name assignment for HttpClientError', async ()=>{
            const invalidClient = {};
            const { result } = renderHook(()=>useAuthenticatedApi(invalidClient));
            try {
                await result.current.authenticatedPost('/test', {
                    data: 'test'
                });
            } catch (error) {
        ...
        it('should verify error.name assignment for InvalidUrlError', async ()=>{
            const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient));
            try {
                await result.current.authenticatedPost('', {
                    data: 'test'
                });
            } catch (error) {
        ...
        it('should verify token check - token is empty string', ()=>{
            ...
            expect(mockGet).toHaveBeenCalledWith(expect.any(String), expect.not.objectContaining({
                Authorization: expect.any(String)
            }));
        });
        ...
        it('should verify url construction with baseUrl ending in slash', ()=>{
            ...
            result.current.authenticatedGet('/endpoint');
            expect(mockGet).toHaveBeenCalledWith('https://api.test//endpoint', expect.any(Object));
        });
        ...
        it('should verify all four methods use same client instance', ()=>{
            const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient));
            result.current.authenticatedPost('/post', {
                data: 'post'
            });
            result.current.authenticatedGet('/get');
            result.current.authenticatedPut('/put', {
                data: 'put'
            });
        ...
        it('should verify useCallback dependencies - client change triggers update', ()=>{
            ...
            const { result, rerender } = renderHook(({ client })=>useAuthenticatedApi(client), {
                initialProps: {
        ...
        it('should verify useCallback dependencies - baseUrl change triggers update', ()=>{
            const { result, rerender } = renderHook(({ baseUrl })=>useAuthenticatedApi(mockHttpClient, baseUrl), {
                initialProps: {
            ...
            expect(mockGet).toHaveBeenCalledWith('https://api2.test/test2', expect.any(Object));
        });
        ...
        it('should verify url.trim() exact comparison - returns non-empty string', ()=>{
            ...
            expect(()=>{
                result.current.authenticatedPost('/endpoint', {
                    data: 'test'
                });
            }).not.toThrow();
        });
        ...
        it('should verify exact client.put() call with correct parameters', async ()=>{
            const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient));
            await result.current.authenticatedPut('/test', {
                data: 'test'
            });
            // Verify exact method call with exact parameters
        ...
        it('should verify exact template literal ${baseUrl}${endpoint} - both values', async ()=>{
            ...
            expect(mockGet).toHaveBeenCalledWith('https://custom-api.com/api/endpoint', expect.any(Object));
            expect(mockGet).not.toHaveBeenCalledWith('https://custom-api.com/apiendpoint', expect.any(Object));
        });
        ...
        it('should verify exact useCallback dependencies - token change', ()=>{
            const { result, rerender } = renderHook(()=>useAuthenticatedApi(mockHttpClient));
            result.current.authenticatedPost('/test1', {
                data: 'test1'
            });
            mockUseAuth.mockReturnValue({
                token: 'new-token',
    ...
    describe('additional edge cases for improved mutation coverage', ()=>{
        ...
        it('should verify exact template literal for URL construction', ()=>{
            ...
            expect(mockGet).toHaveBeenCalledWith('https://custom-api.com/api/test', expect.any(Object));
        });
    ...
    describe('mutation killers - no coverage paths', ()=>{
        describe('createSafeError - error creation fallbacks', ()=>{
            ...
            it('should verify outer catch block in createSafeError - ultimate fallback', async ()=>{
                ...
                await expect(result2.current.authenticatedPost('/test', {
                    data: 'test'
                })).rejects.toThrow(HTTP_CLIENT_ERROR_MSG);
            });
            ...
            it('should verify fallback client methods - get', async ()=>{
                ...
                const { result } = renderHook(()=>useAuthenticatedApi(undefined));
                await expect(result.current.authenticatedGet('/test')).rejects.toThrow('HTTP client initialization failed');
                defaultAdapters.createHttpClient = originalCreateHttpClient;
            });
            ...
            it('should verify fallback client methods - delete', async ()=>{
                ...
                const { result } = renderHook(()=>useAuthenticatedApi(undefined));
                await expect(result.current.authenticatedDelete('/test')).rejects.toThrow('HTTP client initialization failed');
                defaultAdapters.createHttpClient = originalCreateHttpClient;
            });
        ...
        describe('mutation killers - no coverage paths', ()=>{
            ...
            describe('URL validation error paths', ()=>{
                ...
                it('should verify URL validation - whitespace baseUrl and empty endpoint in GET', async ()=>{
                    const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient, '   '));
                    await expect(result.current.authenticatedGet('')).rejects.toThrow(URL_EMPTY_ERROR_MSG);
                });
                ...
                it('should verify URL validation - whitespace baseUrl and empty endpoint in PUT', async ()=>{
                    const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient, '   '));
                    await expect(result.current.authenticatedPut('', {
                        data: 'test'
                    })).rejects.toThrow(URL_EMPTY_ERROR_MSG);
                });
                it('should verify URL validation - whitespace baseUrl and empty endpoint in DELETE', async ()=>{
                    const { result } = renderHook(()=>useAuthenticatedApi(mockHttpClient, '   '));
                    await expect(result.current.authenticatedDelete('')).rejects.toThrow(URL_EMPTY_ERROR_MSG);
                });
            });
        ...
        describe('error handling - non-Error objects', ()=>{
            ...
            it('should handle when error is not an Error instance in GET', async ()=>{
                ...
                const { result } = renderHook(()=>useAuthenticatedApi(errorClient));
                await expect(result.current.authenticatedGet('/test')).rejects.toThrow('object error');
            });
            ...
            it('should handle when error is not an Error instance in DELETE', async ()=>{
                ...
                const { result } = renderHook(()=>useAuthenticatedApi(errorClient));
                await expect(result.current.authenticatedDelete('/test')).rejects.toThrow('Request failed');
            });
        });
