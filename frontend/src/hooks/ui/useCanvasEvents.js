...
export function useCanvasEvents({ reactFlowInstanceRef, setNodes, setEdges, setSelectedNodeId, notifyModified, clipboard, storage }) {
    ...
    const handleAddToAgentNodes = useCallback((node)=>{
        ...
        try {
            ...
            const exists = agentNodes.some((n)=>n.label === agentTemplate.label && JSON.stringify(n.agent_config) === JSON.stringify(agentTemplate.agent_config));
            if (!exists) {
    ...
    const handleAddToToolNodes = useCallback((node)=>{
        ...
        try {
            ...
            const exists = tools.some((n)=>n.label === toolTemplate.label && JSON.stringify(n.tool_config) === JSON.stringify(toolTemplate.tool_config));
            if (!exists) {
