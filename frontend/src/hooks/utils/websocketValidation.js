 ...
 */ export function isValidWebSocket(ws) {
 ...
 */ export function hasPendingReconnection(timeout) {
 ...
 */ export function sanitizeReconnectionDelay(delay) {
    const MIN_DELAY = 1;
 ...
 */ export function isCleanClosure(event) {
 ...
 */ export function getCloseReason(event) {
    if (event.reason !== null && event.reason !== undefined && event.reason.length > 0) {
        return event.reason;
    }
