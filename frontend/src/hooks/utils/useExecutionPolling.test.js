...
describe('useExecutionPolling', ()=>{
    ...
    describe('Timeout Guards', ()=>{
        it('should stop polling after max iterations (1000)', async ()=>{
            ...
            expect(mockLoggerWarn).toHaveBeenCalledWith(expect.stringContaining(`Max polling iterations (${MAX_ITERATIONS}) reached`));
        });
        ...
        it('should limit concurrent executions to 50', async ()=>{
            ...
            mockTabsRef.current = [
                createMockTab('workflow-1', executions)
            ];
            mockApi.getExecution.mockResolvedValue({
                status: 'running'
            });
            ...
            expect(mockLoggerWarn).toHaveBeenCalledWith(expect.stringContaining('Too many running executions (60), limiting to 50'));
        });
    });
});
