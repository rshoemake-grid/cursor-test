...
function logSkipConnectionReason(
  executionId,
  executionStatus,
  lastKnownStatus,
  logger,
  ...
  if (isTemporaryExecutionId(executionId)) {
    ...
    if (
      ...
      logger.debug(
        `[WebSocket] Skipping connection - execution ${executionId} is ${status}`,
      );
    }
  }
}
function logSkipReconnectReason(
  executionId,
  executionStatus,
  lastKnownStatus,
  event,
  ...
  if (isTemporaryExecutionId(executionId)) {
    ...
    if (
      ...
      logger.debug(
        `[WebSocket] Skipping reconnect - execution ${executionId} is ${status}`,
      );
...
export { logSkipConnectionReason, logSkipReconnectReason };
