  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      const runningExecutions = currentTabs.flatMap((tab) => {
        ...
        const hasExecutions = hasTab === true && tab.executions !== null && tab.executions !== void 0;
        ...
        return tab.executions.filter((e) => {
          ...
          const hasId = hasExecution === true && e.id !== null && e.id !== void 0;
      ...
      const updates = await Promise.all(
        runningExecutions.map(async (exec) => {
          try {
            ...
            const newStatus = isCompleted === true ? "completed" : isFailed === true ? "failed" : isPaused === true ? "running" : (
            ...
            const hasCompletedAt = execution.completed_at !== null && execution.completed_at !== void 0;
            const completedAt = hasCompletedAt === true ? new Date(execution.completed_at) : void 0;
            const hasNodeStates = execution.node_states !== null && execution.node_states !== void 0;
            const nodes = hasNodeStates === true ? execution.node_states : {};
            const hasLogs = execution.logs !== null && execution.logs !== void 0 && Array.isArray(execution.logs) === true;
      ...
      setTabs((prev) => {
        return prev.map((tab) => ({
          ...tab,
          executions: (() => {
            const hasExecutions = tab.executions !== null && tab.executions !== void 0;
            const isExecutionsArray = hasExecutions === true && Array.isArray(tab.executions) === true;
            if (hasExecutions === false || isExecutionsArray === false) return [];
            return tab.executions.map((exec) => {
              const update = updates.find((u) => {
                const hasUpdate2 = u !== null && u !== void 0;
                const hasUpdateId = hasUpdate2 === true && u.id !== null && u.id !== void 0;
                const matchesExec = hasUpdateId === true && u.id === exec.id;
                return hasUpdate2 === true && hasUpdateId === true && matchesExec === true;
              });
