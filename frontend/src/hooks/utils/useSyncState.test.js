...
describe('useSyncState', ()=>{
    it('should sync value when source is truthy (default behavior)', ()=>{
        ...
        const { rerender } = renderHook(({ source })=>useSyncState(source, setter), {
            initialProps: {
                source: null
            }
        });
    ...
    it('should use condition function when provided', ()=>{
        const setter = jest.fn();
        const condition = jest.fn((value)=>value !== null && value !== undefined);
        const { rerender } = renderHook(({ source })=>useSyncState(source, setter, condition), {
            initialProps: {
                source: null
            }
        });
...
describe('useSyncStateWithDefault', ()=>{
    it('should sync source value when not null/undefined', ()=>{
        const setter = jest.fn();
        const { rerender } = renderHook(({ source })=>useSyncStateWithDefault(source, setter, null), {
            initialProps: {
                source: null
            }
        });
    ...
    it('should sync empty string as valid value', ()=>{
        const setter = jest.fn();
        const { rerender } = renderHook(({ source })=>useSyncStateWithDefault(source, setter, 'default'), {
            initialProps: {
                source: null
            }
        });
    ...
    it('should sync false as valid value', ()=>{
        const setter = jest.fn();
        const { rerender } = renderHook(({ source })=>useSyncStateWithDefault(source, setter, true), {
            initialProps: {
                source: null
            }
        });
