 ...
 */ import { deleteAgentsFromStorage, extractAgentIds, updateStateAfterDeletion } from './agentDeletionService';
describe('deleteAgentsFromStorage', ()=>{
    ...
    it('should return error when storage is not available', ()=>{
        const result = deleteAgentsFromStorage(null, 'testKey', new Set([
            'agent-1'
        ]), mockCallbacks);
    ...
    it('should return error when storage.getItem returns null', ()=>{
        ...
        const result = deleteAgentsFromStorage(mockStorage, 'testKey', new Set([
            'agent-1'
        ]), mockCallbacks);
        expect(result.success).toBe(false);
    ...
    it('should handle deletion of non-existent agents', ()=>{
        ...
        expect(result.success).toBe(true);
        expect(result.deletedCount).toBe(0);
        expect(mockStorage.setItem).toHaveBeenCalledWith('testKey', JSON.stringify(mockAgents));
    });
    ...
    it('should use errorPrefix in error messages', ()=>{
        ...
        expect(mockCallbacks.showError).toHaveBeenCalledWith(expect.stringContaining('repository agents'));
    });
    it('should handle empty agentIdsToDelete set', ()=>{
        mockStorage.getItem.mockReturnValue(JSON.stringify(mockAgents));
        mockStorage.setItem.mockReturnValue(undefined);
        const result = deleteAgentsFromStorage(mockStorage, 'testKey', new Set(), mockCallbacks);
        expect(result.success).toBe(true);
        expect(result.deletedCount).toBe(0);
        expect(mockStorage.setItem).toHaveBeenCalledWith('testKey', JSON.stringify(mockAgents));
    });
...
describe('extractAgentIds', ()=>{
    ...
    it('should return empty set for null array', ()=>{
        const result = extractAgentIds(null);
        expect(result).toBeInstanceOf(Set);
        expect(result.size).toBe(0);
    });
});
describe('updateStateAfterDeletion', ()=>{
    it('should update agents state by removing deleted IDs', ()=>{
        ...
        updateStateAfterDeletion(new Set([
            'agent-1',
            'agent-3'
        ]), mockSetAgents, mockSetSelectedIds);
