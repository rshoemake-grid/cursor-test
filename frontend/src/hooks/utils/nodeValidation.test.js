 ...
 */ import { isValidNodeId, hasValidCache, nodeExistsAndValid } from './nodeValidation';
...
describe('hasValidCache', ()=>{
    it('should return true when cache is valid', ()=>{
        expect(hasValidCache('node-1', 'node-1', {
            id: 'node-1'
        })).toBe(true);
    });
    it('should return false when IDs do not match', ()=>{
        expect(hasValidCache('node-1', 'node-2', {
            id: 'node-1'
        })).toBe(false);
    });
