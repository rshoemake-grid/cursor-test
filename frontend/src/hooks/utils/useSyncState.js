 ...
 */ export function useSyncState(source, setter, condition) {
 ...
 */ export function useSyncStateWithDefault(source, setter, defaultValue = null) {
    // Use refs to store stable references and prevent infinite loops
    const setterRef = useRef(setter);
    ...
    useEffect(()=>{
        ...
        source
    ]); // Only depend on source to prevent infinite loops
}
