...
describe('websocketLogging', ()=>{
    ...
    beforeEach(()=>{
        jest.clearAllMocks();
        isTemporaryExecutionId.mockReturnValue(false);
    });
    describe('logSkipConnectionReason', ()=>{
        it('should log skip reason for temporary execution ID', ()=>{
            ...
            logSkipConnectionReason(executionId, undefined, undefined, mockLogger);
            expect(mockLogger.debug).toHaveBeenCalledWith(`[WebSocket] Skipping connection to temporary execution ID: ${executionId}`);
        });
        it('should log skip reason when execution is completed', ()=>{
            const executionId = 'exec-123';
            const executionStatus = EXECUTION_STATUS.COMPLETED;
            logSkipConnectionReason(executionId, executionStatus, undefined, mockLogger);
            expect(mockLogger.debug).toHaveBeenCalledWith(`[WebSocket] Skipping connection - execution ${executionId} is ${executionStatus}`);
        });
        ...
        it('should use lastKnownStatus when executionStatus is undefined', ()=>{
            ...
            logSkipConnectionReason(executionId, undefined, lastKnownStatus, mockLogger);
            expect(mockLogger.debug).toHaveBeenCalledWith(`[WebSocket] Skipping connection - execution ${executionId} is ${lastKnownStatus}`);
        });
        it('should prioritize executionStatus over lastKnownStatus', ()=>{
            const executionId = 'exec-123';
            const executionStatus = EXECUTION_STATUS.FAILED;
            const lastKnownStatus = EXECUTION_STATUS.COMPLETED;
            logSkipConnectionReason(executionId, executionStatus, lastKnownStatus, mockLogger);
            expect(mockLogger.debug).toHaveBeenCalledWith(`[WebSocket] Skipping connection - execution ${executionId} is ${executionStatus}`);
        });
        ...
        it('should not log when execution ID is null and not temporary', ()=>{
            logSkipConnectionReason(null, undefined, undefined, mockLogger);
            expect(mockLogger.debug).not.toHaveBeenCalled();
        });
    });
    describe('logSkipReconnectReason', ()=>{
        ...
        it('should log skip reason for temporary execution ID', ()=>{
            ...
            logSkipReconnectReason(executionId, undefined, undefined, event, mockIsCleanClosure, mockLogger);
            expect(mockLogger.debug).toHaveBeenCalledWith(`[WebSocket] Skipping reconnect for temporary execution ID: ${executionId}`);
        });
        it('should log skip reason when execution is completed', ()=>{
            const executionId = 'exec-123';
            const executionStatus = EXECUTION_STATUS.COMPLETED;
            const event = new CloseEvent('close');
            logSkipReconnectReason(executionId, executionStatus, undefined, event, mockIsCleanClosure, mockLogger);
            expect(mockLogger.debug).toHaveBeenCalledWith(`[WebSocket] Skipping reconnect - execution ${executionId} is ${executionStatus}`);
        });
        it('should log skip reason when execution is failed', ()=>{
            const executionId = 'exec-123';
            const executionStatus = EXECUTION_STATUS.FAILED;
        ...
        it('should log skip reason when connection closed cleanly', ()=>{
            ...
            logSkipReconnectReason(executionId, executionStatus, undefined, event, mockIsCleanClosure, mockLogger);
            expect(mockLogger.debug).toHaveBeenCalledWith(`[WebSocket] Connection closed cleanly, not reconnecting`);
        });
        it('should use lastKnownStatus when executionStatus is undefined', ()=>{
            const executionId = 'exec-123';
            const lastKnownStatus = EXECUTION_STATUS.COMPLETED;
            const event = new CloseEvent('close');
            logSkipReconnectReason(executionId, undefined, lastKnownStatus, event, mockIsCleanClosure, mockLogger);
            expect(mockLogger.debug).toHaveBeenCalledWith(`[WebSocket] Skipping reconnect - execution ${executionId} is ${lastKnownStatus}`);
        });
        it('should prioritize executionStatus over lastKnownStatus', ()=>{
            const executionId = 'exec-123';
            const executionStatus = EXECUTION_STATUS.FAILED;
            const lastKnownStatus = EXECUTION_STATUS.COMPLETED;
            const event = new CloseEvent('close');
            logSkipReconnectReason(executionId, executionStatus, lastKnownStatus, event, mockIsCleanClosure, mockLogger);
            expect(mockLogger.debug).toHaveBeenCalledWith(`[WebSocket] Skipping reconnect - execution ${executionId} is ${executionStatus}`);
        });
