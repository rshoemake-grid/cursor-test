...
describe("ReconnectionStrategy", () => {
  describe("ExponentialBackoffStrategy", () => {
    ...
    describe("calculateDelay", () => {
      it("should return baseDelay for attempt 1", () => {
        const delay = strategy.calculateDelay(1, 1e3);
        expect(delay).toBe(1e3);
      });
      it("should return 2x baseDelay for attempt 2", () => {
        const delay = strategy.calculateDelay(2, 1e3);
        expect(delay).toBe(2e3);
      });
      it("should return 4x baseDelay for attempt 3", () => {
        const delay = strategy.calculateDelay(3, 1e3);
        expect(delay).toBe(4e3);
      });
      it("should return 8x baseDelay for attempt 4", () => {
        const delay = strategy.calculateDelay(4, 1e3);
        expect(delay).toBe(8e3);
      });
      it("should cap at MAX_DELAY", () => {
        const delay = strategy.calculateDelay(10, 1e4);
        expect(delay).toBe(WS_RECONNECT.MAX_DELAY);
      });
      it("should return MIN_DELAY for invalid attempt < 1", () => {
        const delay = strategy.calculateDelay(0, 1e3);
        expect(delay).toBe(1e3);
      });
  ...
  describe("LinearBackoffStrategy", () => {
    ...
    describe("calculateDelay", () => {
      ...
      it("should return 3x baseDelay for attempt 3", () => {
        const delay = strategy.calculateDelay(3, 1e3);
        expect(delay).toBe(3e3);
      });
      it("should cap at MAX_DELAY", () => {
        const delay = strategy.calculateDelay(100, 1e3);
        expect(delay).toBe(WS_RECONNECT.MAX_DELAY);
      });
    });
    describe("shouldReconnect", () => {
  ...
  describe("FixedDelayStrategy", () => {
    it("should use default fixed delay when not specified", () => {
      ...
      const delay = strategy.calculateDelay(5, 1e3);
      expect(delay).toBe(WS_RECONNECT.BASE_DELAY);
    });
    it("should use custom fixed delay", () => {
      const strategy = new FixedDelayStrategy(5e3);
      const delay = strategy.calculateDelay(10, 1e3);
      expect(delay).toBe(5e3);
    });
    it("should cap custom delay at MAX_DELAY", () => {
      const strategy = new FixedDelayStrategy(1e5);
      const delay = strategy.calculateDelay(1, 1e3);
      expect(delay).toBe(WS_RECONNECT.MAX_DELAY);
    });
