 ...
 */ export async function confirmUnsavedChanges(onConfirm) {
    const confirmed = await showConfirm('This workflow has unsaved changes. Close anyway?', UNSAVED_CHANGES_OPTIONS);
    if (confirmed) {
        await onConfirm();
    }
}
 ...
 * @returns Promise that resolves when confirmation is complete
 */ export async function confirmDelete(itemName, onConfirm, options) {
 ...
 * @param onConfirm Callback to execute if confirmed
 * @returns Promise that resolves when confirmation is complete
 */ export async function confirmAction(message, options, onConfirm) {
