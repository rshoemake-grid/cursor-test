...
import { ExecutionStatusChecker, isExecutionTerminated, shouldSkipConnection, shouldReconnect } from './executionStatusUtils';
...
describe('Vite/ESM browser compatibility', ()=>{
    it('must not use require() - would cause "require is not defined" in Vite browser build', ()=>{
        const filePath = path.join(__dirname, 'executionStatusUtils.js');
...
describe('ExecutionStatusChecker', ()=>{
    ...
    describe('shouldSkip', ()=>{
        ...
        it('should return false for valid pending execution', ()=>{
            expect(ExecutionStatusChecker.shouldSkip('exec-123', 'pending')).toBe(false);
        });
    ...
    describe('shouldReconnect', ()=>{
        it('should return false for temporary execution IDs', ()=>{
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 1, 3, 'temp-123', EXECUTION_STATUS.RUNNING)).toBe(false);
        });
        it('should return false when execution is terminated', ()=>{
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 1, 3, 'exec-123', EXECUTION_STATUS.COMPLETED)).toBe(false);
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 1, 3, 'exec-123', EXECUTION_STATUS.FAILED)).toBe(false);
        });
        it('should return false when connection was closed cleanly', ()=>{
            expect(ExecutionStatusChecker.shouldReconnect(true, 1000, 1, 3, 'exec-123', EXECUTION_STATUS.RUNNING)).toBe(false);
        });
        it('should return false when max attempts reached', ()=>{
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 3, 3, 'exec-123', EXECUTION_STATUS.RUNNING)).toBe(false);
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 4, 3, 'exec-123', EXECUTION_STATUS.RUNNING)).toBe(false);
        });
        it('should return false when executionId is null', ()=>{
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 1, 3, null, EXECUTION_STATUS.RUNNING)).toBe(false);
        });
        it('should return true for unclean closure with running execution', ()=>{
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 1, 3, 'exec-123', EXECUTION_STATUS.RUNNING)).toBe(true);
        });
        it('should return true for unclean closure with pending execution', ()=>{
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 1, 3, 'exec-123', 'pending')).toBe(true);
        });
        it('should return true when clean closure but code is not 1000', ()=>{
            expect(ExecutionStatusChecker.shouldReconnect(true, 1001, 1, 3, 'exec-123', EXECUTION_STATUS.RUNNING)).toBe(true);
        });
        it('should check lastKnownStatus when status is undefined', ()=>{
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 1, 3, 'exec-123', undefined, EXECUTION_STATUS.COMPLETED)).toBe(false);
            expect(ExecutionStatusChecker.shouldReconnect(false, 1006, 1, 3, 'exec-123', undefined, EXECUTION_STATUS.RUNNING)).toBe(true);
        });
    });
});
describe('Legacy function exports', ()=>{
    ...
    describe('shouldReconnect', ()=>{
        it('should delegate to ExecutionStatusChecker.shouldReconnect', ()=>{
            expect(shouldReconnect(false, 1006, 1, 3, 'exec-123', EXECUTION_STATUS.RUNNING)).toBe(true);
            expect(shouldReconnect(true, 1000, 1, 3, 'exec-123', EXECUTION_STATUS.RUNNING)).toBe(false);
            expect(shouldReconnect(false, 1006, 1, 3, 'temp-123', EXECUTION_STATUS.RUNNING)).toBe(false);
        });
    });
});
