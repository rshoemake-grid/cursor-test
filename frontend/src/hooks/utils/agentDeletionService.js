 ...
 */ export function deleteAgentsFromStorage(storage, storageKey, agentIdsToDelete, callbacks) {
    // Validate storage
 ...
 */ export function extractAgentIds(agents) {
    if (!hasArrayItems(agents)) {
        return new Set();
    }
    return new Set(agents.map((a)=>a && a.id ? a.id : null).filter((id)=>id !== null && id !== undefined));
}
/**
 ...
 */ export function updateStateAfterDeletion(agentIdsToDelete, setAgents, setSelectedIds) {
