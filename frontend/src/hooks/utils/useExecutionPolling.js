...
import { isRealExecutionId, shouldLogExecutionError } from './executionIdValidation';
 ...
 */ export function useExecutionPolling({ tabsRef, setTabs, apiClient, logger: injectedLogger = logger, pollInterval = 2000 }) {
    useEffect(()=>{
        ...
        const interval = setInterval(async ()=>{
            ...
            const updates = await Promise.all(runningExecutions.map(async (exec)=>{
                try {
                    ...
                    const newStatus = isCompleted === true ? 'completed' : isFailed === true ? 'failed' : isPaused === true ? 'running' : 'running';
                    // Explicit boolean check to prevent mutation survivors
