 ...
 */ export function validateRequest(config, context) {
    // Validate client
 ...
 */ export function buildRequestHeaders(token, method, additionalHeaders) {
 ...
 */ export function executeAuthenticatedRequest(config, context) {
    // Validate request
    ...
    // Build headers
    const headers = buildRequestHeaders(context.token, config.method, config.additionalHeaders);
    // Build URL
    const url = `${context.baseUrl}${config.endpoint}`;
    ...
    if (!requestFn) {
        return Promise.reject(createSafeError(`Unsupported HTTP method: ${config.method}`, 'UnsupportedMethodError'));
    }
    try {
        return requestFn();
    } catch (error) {
