  ...
  handleExecutionStart(tabs, activeTabId, executionId) {
    ...
    return tabs.map((tab) => {
      ...
      if (isRealExecutionId(executionId)) {
        const pendingExecutions = tab.executions.map((exec, idx) => ({ exec, idx })).filter(({ exec }) => exec && exec.id && isPendingExecutionId(exec.id));
        if (pendingExecutions.length > 0) {
          const oldestPending = pendingExecutions[pendingExecutions.length - 1];
          return {
            ...tab,
            executions: tab.executions.map(
              (exec, idx) => idx === oldestPending.idx ? {
                ...exec,
                id: executionId
              } : exec
            ),
  ...
  handleExecutionLogUpdate(tabs, workflowId, executionId, log) {
    return updateTabByWorkflowId(tabs, workflowId, {
      executions: logicalOrToEmptyArray(
        tabs.find((tab) => tab.workflowId === workflowId)?.executions.map(
          (exec) => exec.id === executionId ? { ...exec, logs: [...exec.logs, log] } : exec
        )
      )
    });
  }
      ...
      executions: logicalOrToEmptyArray(
        tabs.find((tab) => tab.workflowId === workflowId)?.executions.map(
          (exec) => exec.id === executionId ? {
            ...exec,
            status,
            completedAt: status === "completed" || status === "failed" ? /* @__PURE__ */ new Date() : exec.completedAt
          } : exec
        )
      )
    });
