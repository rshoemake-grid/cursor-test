...
describe('useAutoSave', ()=>{
    ...
    it('should use custom delay', ()=>{
        const saveFn = jest.fn();
        const { rerender } = renderHook(({ value })=>useAutoSave(value, saveFn, 1000), {
            initialProps: {
                value: 'initial'
            }
        });
    ...
    it('should not save when enabled is false', ()=>{
        const saveFn = jest.fn();
        const { rerender } = renderHook(({ value })=>useAutoSave(value, saveFn, 500, false), {
            initialProps: {
                value: 'initial'
            }
        });
    ...
    it('should save when enabled changes to true', ()=>{
        const saveFn = jest.fn();
        const { rerender } = renderHook(({ value, enabled })=>useAutoSave(value, saveFn, 500, enabled), {
            initialProps: {
                value: 'initial',
                enabled: false
            }
    ...
    it('should cleanup timeout on unmount', ()=>{
        const saveFn = jest.fn();
        const { rerender, unmount } = renderHook(({ value })=>useAutoSave(value, saveFn), {
            initialProps: {
                value: 'initial'
            }
        });
