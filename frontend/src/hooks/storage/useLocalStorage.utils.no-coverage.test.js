 ...
 */ import { parseJsonSafely, looksLikeJson, stringifyForStorage, readStorageItem, writeStorageItem, deleteStorageItem, shouldHandleStorageEvent } from './useLocalStorage.utils';
describe('useLocalStorage.utils - No Coverage Paths', ()=>{
    ...
    describe('stringifyForStorage', ()=>{
        it('should stringify regular value', ()=>{
            const result = stringifyForStorage({
                key: 'value'
            });
    ...
    describe('readStorageItem', ()=>{
        ...
        it('should return defaultValue and log warning for invalid JSON that looks like JSON', ()=>{
            ...
            expect(result).toBe('default');
            expect(mockLogger.warn).toHaveBeenCalledWith('localStorage key "key" contains invalid JSON. Returning default value.', '{invalid json}');
        });
        ...
        it('should return defaultValue and log warning for plain string when default is not string', ()=>{
            ...
            expect(result).toBe(42);
            expect(mockLogger.warn).toHaveBeenCalledWith('localStorage key "key" contains plain string but expected JSON. Returning default value.', 'plain string');
        });
        it('should return defaultValue and log error when storage.getItem throws', ()=>{
            ...
            const result = readStorageItem(mockStorage, 'key', 'default', mockLogger);
            expect(result).toBe('default');
            expect(mockLogger.error).toHaveBeenCalledWith('Error reading localStorage key "key":', expect.any(Error));
        });
    ...
    describe('writeStorageItem', ()=>{
        ...
        it('should return false and log error when storage.setItem throws', ()=>{
            ...
            expect(result).toBe(false);
            expect(mockLogger.error).toHaveBeenCalledWith('Error setting localStorage key "key":', expect.any(Error));
        });
    ...
    describe('deleteStorageItem', ()=>{
        ...
        it('should return false and log error when storage.removeItem throws', ()=>{
            ...
            expect(result).toBe(false);
            expect(mockLogger.error).toHaveBeenCalledWith('Error removing localStorage key "key":', expect.any(Error));
        });
