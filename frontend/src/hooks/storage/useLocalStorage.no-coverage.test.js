...
describe('useLocalStorage - No Coverage Paths', ()=>{
    ...
    describe('useLocalStorage - catch blocks', ()=>{
        it('should handle storage.getItem throwing in useState initializer', ()=>{
            ...
            expect(logger.error).toHaveBeenCalledWith('Error reading localStorage key "test-key":', expect.any(Error));
        });
        it('should handle JSON.parse throwing in useState initializer', ()=>{
            ...
            expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('contains invalid JSON'), '{invalid json');
        });
        it('should handle storage.setItem throwing in setValue', ()=>{
            ...
            expect(logger.error).toHaveBeenCalledWith('Error setting localStorage key "test-key":', expect.any(Error));
        ...
        it('should handle storage.removeItem throwing in removeValue', ()=>{
            ...
            expect(logger.error).toHaveBeenCalledWith('Error removing localStorage key "test-key":', expect.any(Error));
        ...
        it('should handle JSON.parse throwing in useEffect storage event handler', ()=>{
            ...
            const handleStorageChange = mockStorage.addEventListener.mock.calls.find((call)=>call[0] === 'storage')?.[1];
    ...
    describe('useLocalStorage - storage event listener', ()=>{
        it('should return early when storage is null (lines 60-61)', ()=>{
            ...
            const { result, rerender } = renderHook(({ storage })=>useLocalStorage('test-key', 'default', {
                    storage
                }), {
                initialProps: {
                    storage: null
            ...
            const addEventListenerCalls = mockStorage.addEventListener.mock.calls.filter((call)=>call[0] === 'storage');
        ...
        it('should update storedValue when parsed value is not null (lines 67-68)', ()=>{
            ...
            act(()=>{
                ...
                const handler = mockStorage.addEventListener.mock.calls.find((call)=>call[0] === 'storage')?.[1];
                if (handler) {
