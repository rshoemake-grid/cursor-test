    ...
    describe('error handling', ()=>{
        it('should handle WebSocket connection errors', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('[WebSocket] Connection error for execution exec-1:'), expect.objectContaining({
        ...
        it('should verify exact comparison error instanceof Error - error is Error instance', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                const errorCalls = logger.error.mock.calls.filter((call)=>call[0]?.includes('[WebSocket] Connection error'));
        ...
        it('should verify exact comparison reason && reason.length > 0 - reason exists and not empty', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('[WebSocket] Disconnected'), expect.objectContaining({
        ...
        it('should verify exact comparison reconnectAttempts.current >= maxReconnectAttempts - exact boundary', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                const warnCalls = logger.warn.mock.calls.filter((call)=>call[0]?.includes('Max reconnect attempts'));
        ...
        it('should verify exact Math.min and Math.pow calculation for reconnect delay', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                const debugCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Reconnecting in'));
        ...
        it('should verify exact comparison error instanceof Error in catch block - error is Error', async ()=>{
            ...
            expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to create connection'), expect.any(Error));
        });
