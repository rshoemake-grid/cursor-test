    ...
    describe('edge cases', ()=>{
        it('should handle close event with no reason', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                await advanceTimersByTime(100);
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('[WebSocket] Disconnected from execution exec-1'), expect.objectContaining({
    ...
    describe('close event reason edge cases', ()=>{
        it('should handle close with empty reason string', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                await advanceTimersByTime(50);
                expect(logger.debug).toHaveBeenCalled();
                const closeCall = logger.debug.mock.calls.find((call)=>call[0]?.includes('Disconnected'));
