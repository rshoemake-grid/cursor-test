...
describe('useWebSocket - edges.comprehensive.1', ()=>{
    ...
    describe('error instanceof edge cases', ()=>{
        it('should handle Error instance in error event', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                const errorCall = logger.error.mock.calls.find((call)=>call[0]?.includes('Connection error'));
    ...
    describe('callback dependency edge cases', ()=>{
        it('should handle connect callback being recreated', async ()=>{
            ...
            const { rerender } = renderHook(({ onLog })=>useWebSocket({
                    executionId: 'exec-1',
                    onLog
                }), {
                initialProps: {
    ...
    describe('comprehensive conditional branch coverage', ()=>{
        ...
        it('should handle instanceof Error check edge cases', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                await advanceTimersByTime(50);
                expect(logger.error).toHaveBeenCalled();
                const errorCall1 = logger.error.mock.calls.find((call)=>call[0]?.includes('Connection error'));
                ...
                await advanceTimersByTime(50);
                expect(logger.error).toHaveBeenCalled();
                const errorCall2 = logger.error.mock.calls.find((call)=>call[0]?.includes('Connection error'));
        ...
        it('should handle reason || fallback edge case', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                await advanceTimersByTime(50);
                expect(logger.debug).toHaveBeenCalled();
                const closeCall1 = logger.debug.mock.calls.find((call)=>call[0]?.includes('Disconnected'));
                ...
                await advanceTimersByTime(50);
                expect(logger.debug).toHaveBeenCalled();
                const closeCall2 = logger.debug.mock.calls.find((call)=>call[0]?.includes('Disconnected'));
