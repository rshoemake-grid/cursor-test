...
describe('useExecutionManagement', ()=>{
    ...
    describe('handleExecutionStart', ()=>{
        it('should return early when no active tab', ()=>{
            const { result } = renderHook(()=>useExecutionManagement({
                    tabs: [],
                    activeTabId: null,
                    setTabs: mockSetTabs,
                    tabsRef: mockTabsRef,
        ...
        it('should create new execution when none exist', ()=>{
            ...
            const updatedTabs = typeof setTabsCall === 'function' ? setTabsCall([
                mockTab
            ]) : setTabsCall;
    ...
    describe('handleExecutionNodeUpdate', ()=>{
        ...
        it('should preserve existing node states', ()=>{
            ...
            act(()=>{
                result.current.handleExecutionNodeUpdate('workflow-1', 'exec-1', 'node-2', {
                    status: 'completed'
                });
            });
            const setTabsCall = mockSetTabs.mock.calls[0][0];
            ...
            const updatedTab = updatedTabs.find((t)=>t.id === 'tab-1');
            const updatedExecution = updatedTab.executions.find((e)=>e.id === 'exec-1');
            expect(updatedExecution.nodes['node-1']).toEqual({
                status: 'running'
            });
            expect(updatedExecution.nodes['node-2']).toEqual({
                status: 'completed'
            });
        });
    });
    describe('polling effect', ()=>{
        it('should not poll when no running executions', async ()=>{
            renderHook(()=>useExecutionManagement({
                    tabs: [
                        mockTab
                    ],
                    activeTabId: 'tab-1',
        ...
        it('should handle execution status changes', async ()=>{
            ...
            const setTabsCall = mockSetTabs.mock.calls.find((call)=>{
                const updatedTabs = typeof call[0] === 'function' ? call[0]([
                    tabWithExecution
                ]) : call[0];
                const updatedTab = updatedTabs.find((t)=>t.id === 'tab-1');
        ...
        it('should clean up interval on unmount', ()=>{
            ...
            const { unmount } = renderHook(()=>useExecutionManagement({
                    tabs: [
                        tabWithExecution
                    ],
                    activeTabId: 'tab-1',
    ...
    describe('edge cases and error handling', ()=>{
        ...
        describe('handleExecutionNodeUpdate edge cases', ()=>{
            it('should handle executionId that does not exist', ()=>{
                ...
                act(()=>{
                    result.current.handleExecutionNodeUpdate('workflow-1', 'non-existent-exec', 'node-1', {
                        status: 'running'
                    });
                });
                // setTabs is still called, but execution is not updated
            ...
            it('should handle workflowId that does not match', ()=>{
                ...
                act(()=>{
                    result.current.handleExecutionNodeUpdate('wrong-workflow', 'exec-1', 'node-1', {
                        status: 'running'
                    });
                });
                // setTabs is still called, but tab is not updated because workflowId doesn't match
    ...
    describe('mutation killers for polling useEffect', ()=>{
        ...
        it('should verify exact logger.debug message for status change', async ()=>{
            ...
            expect(mockLoggerDebug).toHaveBeenCalledWith(expect.stringMatching(/\[WorkflowTabs\] Execution exec-1 status changed: running → completed/));
        });
        it('should verify exact logger.debug message for polling', async ()=>{
            ...
            expect(mockLoggerDebug).toHaveBeenCalledWith(expect.stringMatching(/\[WorkflowTabs\] Polling \d+ running execution\(s\) \(fallback\)\.\.\./));
        });
        it('should verify error handling for 404 on non-pending execution', async ()=>{
            ...
            expect(mockLoggerError).toHaveBeenCalledWith(expect.stringContaining('[WorkflowTabs] Failed to fetch execution exec-1:'), error);
        });
        it('should verify error handling skips logging for pending execution', async ()=>{
            ...
            const errorLogs = mockLoggerError.mock.calls.filter((call)=>call[0] && typeof call[0] === 'string' && call[0].includes('pending-123'));
    ...
    describe('additional coverage for no-coverage mutants', ()=>{
        ...
        describe('useEffect polling - edge cases', ()=>{
            ...
            it('should handle exec.status !== newStatus exact comparison', async ()=>{
                ...
                expect(mockLoggerDebug).toHaveBeenCalledWith(expect.stringContaining('status changed: running → completed'));
            });
            it('should handle exec.status === newStatus (no change)', async ()=>{
                ...
                const statusChangeLogs = mockLoggerDebug.mock.calls.filter((call)=>call[0] && typeof call[0] === 'string' && call[0].includes('status changed'));
