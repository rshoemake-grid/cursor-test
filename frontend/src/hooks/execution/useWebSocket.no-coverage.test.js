...
import { advanceTimersByTime, wsInstances, useWebSocket, logger } from './useWebSocket.test.setup';
        ...
        it('should verify exact comparison - wasClean && code === 1000', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                if (ws.onclose) {
                    ...
                    expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Connection closed cleanly'));
                }
            }
        });
        ...
        it('should handle JSON.parse throwing in onmessage', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                if (ws.onmessage) {
                    ...
                    expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('[WebSocket] Failed to parse message'), expect.any(Error));
                }
            }
        });
