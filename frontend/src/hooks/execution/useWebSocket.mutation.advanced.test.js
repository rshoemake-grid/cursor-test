    ...
    describe('message handler edge cases for 100% coverage', ()=>{
        ...
        it('should verify reason && reason.length > 0 check - reason is empty string', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Disconnected from execution exec-1'), expect.objectContaining({
        ...
        it('should verify currentStatus = executionStatus || lastKnownStatusRef.current - lastKnownStatusRef path', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Skipping reconnect - execution exec-1 is completed'));
            }
        });
        ...
        it('should verify currentStatus === completed || currentStatus === failed - failed path', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Skipping reconnect - execution exec-1 is failed'));
            }
        });
        it('should verify wasClean && code === 1000 check - wasClean is false', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Reconnecting in'));
            }
        });
        ...
        it('should verify reconnectAttempts.current < maxReconnectAttempts && executionId - executionId is null', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                const reconnectLogs = logger.debug.mock.calls.filter((call)=>call[0] && typeof call[0] === 'string' && call[0].includes('Reconnecting in'));
        ...
        it('should verify wsState === WebSocket.CONNECTING branch', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Connection error for execution exec-1'), expect.objectContaining({
        ...
        it('should verify error instanceof Error check - error is Error instance', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                await act(async ()=>{
                    ...
                    if (ws.onerror) {
                        ws.onerror(error); // Pass Error directly as event
                    }
                    await advanceTimersByTime(50);
                });
                expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Connection error for execution exec-1'), expect.objectContaining({
        ...
        it('should verify error instanceof Error check in catch - error is Error', async ()=>{
            ...
            await advanceTimersByTime(100);
            expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to create connection for execution exec-1'), expect.any(Error));
        });
        ...
        it('should verify executionId && executionId.startsWith in onclose - executionId is null', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).not.toHaveBeenCalledWith(expect.stringContaining('Skipping reconnect for temporary execution ID'));
            }
        });
        ...
        it('should verify wasClean && code === 1000 - both conditions true', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Connection closed cleanly, not reconnecting'));
            }
        });
        it('should verify wasClean && code === 1000 - wasClean is false', async ()=>{
            renderHook(()=>useWebSocket({
            ...
            if (wsInstances.length > 0) {
                ...
                const hasReconnectCall = debugCalls.some((msg)=>typeof msg === 'string' && msg.includes('Reconnecting in'));
        ...
        it('should verify Math.min(1000 * Math.pow(2, reconnectAttempts.current), 10000) - delay less than 10000', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringMatching(/\[WebSocket\] Reconnecting in 10000ms \(attempt \d+\/5\)/));
            }
        });
        ...
        it('should verify string literal Execution completed exact value', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                if (closeCalls.length > 0) {
                    const hasExecutionCompleted = closeCalls.some((call)=>call.reason === 'Execution completed');
        ...
        it('should verify setIsConnected(false) exact function call in onerror', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.error.mock.calls.length).toBeGreaterThan(initialErrorCalls);
            }
        });
        ...
        describe('exact string literals and fallback values', ()=>{
            it('should verify exact string literal pending- in startsWith check', async ()=>{
                ...
                const hasPendingMessage = debugCalls.some((call)=>call[0] && typeof call[0] === 'string' && call[0].includes('pending-'));
            ...
            it('should verify exact string literal No reason provided', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).not.toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
                        reason: 'no reason provided'
                    }));
                }
            });
            ...
            it('should verify exact string literal WebSocket connection failed after', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    const errorCalls = onError.mock.calls.filter((call)=>call[0] && typeof call[0] === 'string' && call[0].includes('WebSocket connection failed after'));
            ...
            it('should verify exact comparison wsState === WebSocket.CONNECTING', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    const stateText = state === MockWebSocket.CONNECTING ? 'CONNECTING' : state === MockWebSocket.OPEN ? 'OPEN' : state === MockWebSocket.CLOSING ? 'CLOSING' : state === MockWebSocket.CLOSED ? 'CLOSED' : 'UNKNOWN';
        ...
        describe('additional edge cases for improved mutation coverage', ()=>{
            it('should verify exact wasClean && code === 1000 check - wasClean is false', async ()=>{
                renderHook(({ executionId })=>useWebSocket({
                        executionId
                    }), {
                    initialProps: {
                        executionId: 'exec-1'
        ...
        describe('additional coverage for no-coverage mutants', ()=>{
            describe('exact string literal comparisons', ()=>{
                ...
                it('should verify exact string literal "No reason provided"', async ()=>{
                    ...
                    if (wsInstances.length > 0) {
                        ...
                        const debugCalls = logger.debug.mock.calls;
                        const noReasonCall = debugCalls.find((call)=>call[1] && call[1].reason === 'No reason provided');
                ...
                it('should verify exact string literal "Unknown WebSocket error"', async ()=>{
                    ...
                    expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to create connection'), expect.anything());
                });
            });
            describe('exact WebSocket state comparisons', ()=>{
                it('should verify exact comparison wsState === WebSocket.CONNECTING', async ()=>{
                    ...
                    if (wsInstances.length > 0) {
                        ...
                        expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Connection error'), expect.objectContaining({
                            readyState: 'CONNECTING'
                        }));
                    }
                });
            ...
            describe('exact logical operators', ()=>{
                ...
                it('should verify exact logical AND reason && reason.length > 0', async ()=>{
                    ...
                    if (wsInstances.length > 0) {
                        ...
                        expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Disconnected'), expect.objectContaining({
            ...
            describe('exact instanceof check', ()=>{
                it('should verify exact instanceof Error check', async ()=>{
                    ...
                    if (wsInstances.length > 0) {
                        ...
                        const hasErrorCall = errorCalls.some((call)=>call[0] && typeof call[0] === 'string' && call[0].includes('Connection error'));
            ...
            describe('exact method calls', ()=>{
                ...
                it('should verify exact method call executionId.startsWith(\'pending-\')', async ()=>{
                    ...
                    const debugCalls = logger.debug.mock.calls;
                    const pendingCall = debugCalls.find((call)=>call[0] && typeof call[0] === 'string' && call[0].includes('pending-test'));
