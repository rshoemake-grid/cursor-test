...
describe('useWebSocket - mutation.basic', ()=>{
    ...
    describe('mutation killers for status checks and reconnection', ()=>{
        ...
        it('should verify currentStatus === failed check in connect', async ()=>{
            ...
            expect(wsInstances.length).toBe(0);
            expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Skipping connection - execution exec-1 is failed'));
        });
        ...
        it('should verify exact logger.debug message for connection', async ()=>{
            ...
            expect(logger.debug).toHaveBeenCalledWith('[WebSocket] Connecting to wss://example.com:8000/ws/executions/exec-1');
        });
        it('should verify exact logger.debug message for connected', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith('[WebSocket] Connected to execution exec-1');
            }
        });
        ...
        it('should verify exact logger.debug message for skipping completed execution', async ()=>{
            ...
            await advanceTimersByTime(100);
            // Verify exact log message
            expect(logger.debug).toHaveBeenCalledWith('[WebSocket] Skipping connection - execution exec-1 is completed');
        });
        it('should verify exact logger.debug message for skipping failed execution', async ()=>{
            ...
            await advanceTimersByTime(100);
            // Verify exact log message
            expect(logger.debug).toHaveBeenCalledWith('[WebSocket] Skipping connection - execution exec-1 is failed');
        });
        it('should verify exact logger.debug message for skipping reconnect temporary ID', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                // Verify exact log message
                expect(logger.debug).toHaveBeenCalledWith('[WebSocket] Skipping reconnect for temporary execution ID: pending-123');
            }
        });
        ...
        it('should verify exact logger.debug message for reconnecting', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringMatching(/\[WebSocket\] Reconnecting in \d+ms \(attempt \d+\/5\)/));
            }
        });
        ...
        it('should verify exact logger.error message for connection error', async ()=>{
            ...
            if (wsInstances.length > 0 && wsInstances[0].onerror) {
                ...
                expect(logger.error).toHaveBeenCalledWith('[WebSocket] Connection error for execution exec-1:', expect.objectContaining({
        ...
        it('should verify exact logger.error message for creation failure', async ()=>{
            ...
            expect(logger.error).toHaveBeenCalledWith('Failed to create connection for execution exec-1', expect.any(Error));
        });
