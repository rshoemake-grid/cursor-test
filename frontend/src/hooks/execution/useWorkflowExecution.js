...
import { isUserAuthenticated, hasWorkflowId, isConfirmed, isWorkflowSaved, canExecuteWorkflow } from '../utils/workflowExecutionValidation';
import { extractApiErrorMessage } from '../utils/apiUtils';
export function useWorkflowExecution({ isAuthenticated, localWorkflowId, workflowIdRef, saveWorkflow, onExecutionStart, showSuccess = defaultShowSuccess, showError = defaultShowError, showConfirm = defaultShowConfirm, api = defaultApi, logger = defaultLogger }) {
    ...
    const executeWorkflow = useCallback(async ()=>{
        ...
        if (!hasWorkflowId(currentWorkflowId)) {
            ...
            const confirmed = await showConfirm('Workflow needs to be saved before execution. Save now?', {
                title: 'Save Workflow',
                confirmText: 'Save',
                cancelText: 'Cancel'
            });
