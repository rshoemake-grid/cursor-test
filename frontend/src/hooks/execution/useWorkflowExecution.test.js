...
const waitForWithTimeout = async (callback, timeout = 2000)=>{
...
describe('useWorkflowExecution', ()=>{
    ...
    describe('executeWorkflow', ()=>{
        ...
        it('should prompt to save if no workflow ID exists', async ()=>{
            ...
            expect(mockShowConfirm).toHaveBeenCalledWith('Workflow needs to be saved before execution. Save now?', expect.any(Object));
    ...
    describe('handleConfirmExecute', ()=>{
        ...
        it('should not update execution ID when it matches temp ID', async ()=>{
            ...
            mockApi.executeWorkflow.mockImplementation(async ()=>{
                ...
                return {
                    execution_id: tempExecutionId || 'pending-123'
                };
            });
            const { result } = renderHook(()=>useWorkflowExecution({
                    isAuthenticated: true,
    ...
    describe('edge cases for 100% coverage', ()=>{
        ...
        it('should verify error.response?.data?.detail || error.message || Unknown error - error.response.data.detail path', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('Custom error detail'));
        });
        it('should verify error.response?.data?.detail || error.message || Unknown error - error.message path', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('Error message'));
        });
        ...
        it('should verify error.response?.status and error.response?.data logging', async ()=>{
            ...
            expect(mockLoggerError).toHaveBeenCalledWith('[WorkflowBuilder] Execution failed:', expect.any(Object));
        });
        ...
        it('should verify error.response?.status logging', async ()=>{
            ...
            expect(mockLoggerError).toHaveBeenCalledWith('[WorkflowBuilder] Execution failed:', expect.objectContaining({
        ...
        it('should verify exact string literal Workflow needs to be saved before execution. Save now?', async ()=>{
            ...
            // Verify exact string literal (not mutated)
            expect(mockShowConfirm).toHaveBeenCalledWith('Workflow needs to be saved before execution. Save now?', expect.any(Object));
            expect(mockShowConfirm).not.toHaveBeenCalledWith('Workflow needs to be saved before execution. Save now', expect.any(Object));
        });
        ...
        it('should verify exact logical AND execution.execution_id && execution.execution_id !== tempExecutionId - execution_id equals tempExecutionId', async ()=>{
            ...
            await waitForWithTimeout(()=>{
                ...
                const duplicateCalls = calls.filter((call, index)=>calls.findIndex((c)=>c[0] === call[0]) !== index);
        ...
        it('should verify exact logical OR error.response?.data?.detail || error.message || Unknown error - all three paths', async ()=>{
            ...
            const { result: result1 } = renderHook(()=>useWorkflowExecution({
                    isAuthenticated: true,
                    localWorkflowId: 'workflow-id',
                    workflowIdRef: mockWorkflowIdRef,
                    saveWorkflow: mockSaveWorkflow,
            ...
            jest.clearAllMocks();
            const { result: result2 } = renderHook(()=>useWorkflowExecution({
                    isAuthenticated: true,
                    localWorkflowId: 'workflow-id',
                    workflowIdRef: mockWorkflowIdRef,
                    saveWorkflow: mockSaveWorkflow,
            ...
            jest.clearAllMocks();
            const { result: result3 } = renderHook(()=>useWorkflowExecution({
                    isAuthenticated: true,
                    localWorkflowId: 'workflow-id',
                    workflowIdRef: mockWorkflowIdRef,
                    saveWorkflow: mockSaveWorkflow,
        ...
        // Skipped: Timing-dependent test that fails in Stryker sandbox
        ...
        it('should verify exact showSuccess call with exact message and duration', async ()=>{
            ...
            expect(mockShowSuccess).toHaveBeenCalledWith('✅ Execution starting...\n\nCheck the console at the bottom of the screen to watch it run.', 6000);
            expect(mockShowSuccess).toHaveBeenCalledWith(expect.stringContaining('Execution starting'), 6000);
        });
        it('should verify exact showConfirm call with exact message and options', async ()=>{
            ...
            expect(mockShowConfirm).toHaveBeenCalledWith('Workflow needs to be saved before execution. Save now?', {
                title: 'Save Workflow',
                confirmText: 'Save',
                cancelText: 'Cancel'
            });
        ...
        it('should verify exact workflowIdRef.current property access', async ()=>{
            ...
            expect(mockApi.executeWorkflow).toHaveBeenCalledWith('workflow-id-ref', expect.any(Object));
        });
        // Fixed: Made resilient to Stryker instrumentation state reset timing
        ...
        it('should verify exact logger.debug call with exact message format', async ()=>{
            ...
            expect(mockLoggerDebug).toHaveBeenCalledWith('[WorkflowBuilder] executeWorkflow called');
            expect(mockLoggerDebug).toHaveBeenCalledWith('[WorkflowBuilder] Setting execution inputs and showing dialog');
        });
        ...
        it('should verify exact useCallback dependencies - executeWorkflow', ()=>{
            const { result, rerender } = renderHook(({ isAuthenticated, localWorkflowId })=>useWorkflowExecution({
                    isAuthenticated,
                    localWorkflowId,
                    workflowIdRef: mockWorkflowIdRef,
                    saveWorkflow: mockSaveWorkflow,
        ...
        it('should verify exact workflowIdRef.current property access', async ()=>{
            ...
            expect(mockApi.executeWorkflow).toHaveBeenCalledWith('ref-workflow-id', expect.any(Object));
        });
        ...
        it('should verify exact useCallback dependencies array - executeWorkflow', ()=>{
            const { result, rerender } = renderHook(({ isAuthenticated, localWorkflowId, saveWorkflow })=>useWorkflowExecution({
                    isAuthenticated,
                    localWorkflowId,
                    workflowIdRef: mockWorkflowIdRef,
                    saveWorkflow,
        ...
        it('should verify exact useCallback dependencies array - handleConfirmExecute', ()=>{
            const { result, rerender } = renderHook(({ workflowIdRef, onExecutionStart })=>useWorkflowExecution({
                    isAuthenticated: true,
                    localWorkflowId: 'workflow-id',
                    workflowIdRef,
                    saveWorkflow: mockSaveWorkflow,
            ...
            const newWorkflowIdRef = {
                current: 'new-workflow-id'
            };
            rerender({
                executionInputs: '{"key": "value"}',
        ...
        describe('conditional expression mutation killers', ()=>{
            ...
            it('should verify exact if (execution.execution_id && execution.execution_id !== tempExecutionId) - second false', async ()=>{
                ...
                const tempIdCall = mockOnExecutionStart.mock.calls.find((call)=>tempIdPattern.test(call[0]));
                expect(tempIdCall).toBeDefined();
                ...
                await waitForWithTimeout(()=>{
                    ...
                    const tempIdCalls = mockOnExecutionStart.mock.calls.filter((call)=>tempIdPattern.test(call[0]));
                    ...
                    const execIdCalls = mockOnExecutionStart.mock.calls.filter((call)=>call[0] === tempExecutionId && !tempIdPattern.test(call[0]));
                    expect(execIdCalls.length).toBe(0);
                });
            });
        });
        describe('logical operator mutation killers', ()=>{
            ...
            it('should verify exact Math.random().toString(36).substr(2, 9) in temp ID generation', async ()=>{
                ...
                const tempIdCall = mockOnExecutionStart.mock.calls.find((call)=>typeof call[0] === 'string' && call[0].startsWith('pending-'));
                expect(tempIdCall).toBeDefined();
    ...
    describe('mutation killers - optional chaining in error logging', ()=>{
        ...
        it('should verify exact optional chaining: error?.message in catch block', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('Invalid JSON in execution inputs'));
            expect(mockLoggerError).toHaveBeenCalledWith('[WorkflowExecution] Failed to parse inputs:', errorWithoutMessage);
        });
    ...
    describe('mutation killers - string literals', ()=>{
        it('should verify exact string literal: "Execution setup failed:"', async ()=>{
            ...
            expect(mockLoggerError).toHaveBeenCalledWith('[WorkflowExecution] Failed to parse inputs:', expect.any(Error));
        });
        it('should verify exact string literal: "[WorkflowBuilder] Execution failed:"', async ()=>{
            ...
            expect(mockLoggerError).toHaveBeenCalledWith('[WorkflowBuilder] Execution failed:', expect.any(Error));
        });
    ...
    describe('mutation killers - exact conditionals and optional chaining', ()=>{
        describe('executeWorkflow - exact conditional checks', ()=>{
            it('should verify exact conditional: if (!isAuthenticated)', async ()=>{
                ...
                // Verify exact falsy check: if (!isAuthenticated)
                expect(mockShowError).toHaveBeenCalledWith('Please log in to execute workflows.');
                expect(result.current.showInputs).toBe(false);
            });
            it('should verify exact conditional: if (!currentWorkflowId)', async ()=>{
                ...
                // Verify exact falsy check: if (!currentWorkflowId)
                expect(mockShowConfirm).toHaveBeenCalled();
                expect(mockSaveWorkflow).toHaveBeenCalled();
                expect(result.current.showInputs).toBe(true);
            });
            it('should verify exact conditional: if (!confirmed)', async ()=>{
                ...
                // Verify exact falsy check: if (!confirmed)
                expect(mockShowConfirm).toHaveBeenCalled();
                expect(mockSaveWorkflow).not.toHaveBeenCalled();
                expect(result.current.showInputs).toBe(false);
            });
            it('should verify exact conditional: if (!savedId)', async ()=>{
                ...
                // Verify exact falsy check: if (!savedId)
                expect(mockShowError).toHaveBeenCalledWith('Failed to save workflow. Cannot execute.');
                expect(result.current.showInputs).toBe(false);
            });
        });
        describe('handleConfirmExecute - exact conditional checks', ()=>{
            it('should verify exact conditional: if (!workflowIdToExecute)', async ()=>{
                ...
                // Verify exact falsy check: if (!workflowIdToExecute)
                expect(mockShowError).toHaveBeenCalledWith('Workflow must be saved before executing.');
                expect(result.current.isExecuting).toBe(false);
            });
            it('should verify exact conditional: if (onExecutionStart)', async ()=>{
                ...
                // Verify exact truthy check: if (onExecutionStart)
                // Should be called twice: once with temp ID, once with real execution_id
                expect(mockOnExecutionStart).toHaveBeenCalled();
                expect(mockOnExecutionStart).toHaveBeenCalledWith('exec-123');
            });
            it('should verify exact conditional: if (execution && execution.execution_id && execution.execution_id !== tempExecutionId)', async ()=>{
                ...
                // Verify exact conditional: execution && execution.execution_id && execution.execution_id !== tempExecutionId
                // Should call onExecutionStart with the real execution_id (not temp)
                // The check verifies all three conditions:
                // 1. execution exists
                // 2. execution.execution_id exists
                // 3. execution.execution_id !== tempExecutionId
                expect(mockOnExecutionStart).toHaveBeenCalled();
                // Should be called with real execution_id (different from temp)
                const calls = mockOnExecutionStart.mock.calls;
            ...
            it('should verify exact conditional: execution.execution_id === tempExecutionId (should not update)', async ()=>{
                ...
                // Capture the temp execution ID that will be generated
                let tempExecutionId = null;
                mockOnExecutionStart.mockImplementation((id)=>{
                    if (!tempExecutionId) {
                        tempExecutionId = id; // Capture first call (temp ID)
                    }
                });
                // First call creates temp ID
                await act(async ()=>{
                    await result.current.handleConfirmExecute();
                });
                await act(async ()=>{
                ...
                if (tempExecutionId) {
                    ...
                    // Should not call onExecutionStart again when execution_id === tempExecutionId
                    // The check is: execution.execution_id !== tempExecutionId
                    // When they're equal, the condition is false, so onExecutionStart is not called again
                    // However, onExecutionStart is called once with temp ID at the start
                    // So we verify it was only called once (with temp ID), not twice
                    const calls = mockOnExecutionStart.mock.calls;
                    // Should have been called once with temp ID (from the second execution)
                    // But not with the execution_id since it matches temp ID
        ...
        describe('handleConfirmExecute - exact logical OR operators', ()=>{
            it('should verify exact logical OR: error?.response?.data?.detail || error?.message || "Unknown error" - all branches', async ()=>{
                ...
                // Test branch 1: error?.response?.data?.detail exists
                mockApi.executeWorkflow.mockRejectedValueOnce({
                    response: {
                        data: {
                ...
                // Test branch 2: error?.response?.data?.detail is missing, error?.message exists
                mockShowError.mockClear();
                mockApi.executeWorkflow.mockRejectedValueOnce({
                    message: 'Network Error'
                });
                ...
                expect(mockShowError).toHaveBeenCalledWith('Failed to execute workflow: Network Error');
                // Test branch 3: both missing, uses "Unknown error"
                mockShowError.mockClear();
                mockApi.executeWorkflow.mockRejectedValueOnce({});
                await act(async ()=>{
                    await result.current.handleConfirmExecute();
            ...
            it('should verify exact logical OR: error?.message || "Unknown error" in outer catch', async ()=>{
                ...
                act(()=>{
                    result.current.setExecutionInputs('invalid json{'); // Will cause JSON.parse to throw
                });
                // Test branch 1: error?.message exists
                await act(async ()=>{
                    await result.current.handleConfirmExecute();
                });
                await act(async ()=>{
                ...
                expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('Failed to execute workflow:'));
                // Test branch 2: error?.message is missing
                mockShowError.mockClear();
                act(()=>{
                    result.current.setExecutionInputs('{}');
                });
        ...
        describe('handleConfirmExecute - exact optional chaining', ()=>{
            ...
            it('should verify exact optional chaining: error?.response?.data?.detail', async ()=>{
                ...
                // Should use error.message when error.response is missing
                expect(mockShowError).toHaveBeenCalledWith('Failed to execute workflow: Network Error');
            });
        });
    });
