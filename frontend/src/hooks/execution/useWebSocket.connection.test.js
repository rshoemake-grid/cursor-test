...
describe('useWebSocket - connection', ()=>{
    ...
    describe('connection state', ()=>{
        it('should set isConnected to true when connection opens', async ()=>{
            ...
            expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('[WebSocket] Connecting to'));
            expect(result.current.isConnected).toBe(false);
            ...
            expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('[WebSocket] Connected to execution exec-1'));
            await act(async ()=>{
