...
describe('useWebSocket - edges.comprehensive.2', ()=>{
    ...
    describe('template literal string coverage', ()=>{
        ...
        it('should verify exact logger.debug message for skipping connection', async ()=>{
            ...
            expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Skipping connection - execution exec-1 is completed'));
        });
        it('should verify exact logger.debug message for connecting', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                const connectCall = debugCalls.find((call)=>call[0]?.includes('Connecting to'));
        ...
        it('should verify exact logger.debug message for connected', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                // Verify exact message format
                expect(logger.debug).toHaveBeenCalledWith('[WebSocket] Connected to execution exec-connected-test');
            }
        });
        it('should verify exact logger.debug message for disconnected', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                await advanceTimersByTime(50);
                // Verify exact message format
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('Disconnected from execution exec-disconnect-test'), expect.objectContaining({
                    code: 1000,
                    reason: 'Test reason',
                    wasClean: true
                }));
        ...
        it('should verify exact logger.debug message for skipping reconnect', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                await advanceTimersByTime(50);
                // Verify exact message format
                expect(logger.debug).toHaveBeenCalledWith('[WebSocket] Skipping reconnect for temporary execution ID: pending-reconnect-test');
            }
        });
        it('should verify exact logger.debug message for skipping reconnect with status', async ()=>{
            const executionId = 'exec-status-test';
            const { rerender } = renderHook(({ executionStatus })=>useWebSocket({
                    executionId,
                    executionStatus
                }), {
                initialProps: {
            ...
            if (wsInstances.length > 0) {
                ...
                const reconnectCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Skipping reconnect'));
        ...
        it('should verify exact logger.debug message for clean close', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                await advanceTimersByTime(50);
                // Verify exact message format
                expect(logger.debug).toHaveBeenCalledWith('[WebSocket] Connection closed cleanly, not reconnecting');
            }
        });
        it('should verify exact logger.debug message for reconnecting', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringMatching(/Reconnecting in \d+ms \(attempt \d+\/5\)/));
            }
        });
        ...
        it('should verify exact logger.error message for connection error', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                const errorCalls = logger.error.mock.calls.filter((call)=>call[0]?.includes('Connection error'));
        ...
        it('should verify exact logger.error message for failed to create connection', async ()=>{
            ...
            expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to create connection for execution'), expect.any(Error));
            // Restore
        ...
        it('should verify exact logger.debug message for closing connection', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                // Verify exact message format (code path verification)
                expect(logger.debug).toHaveBeenCalled();
                const closeCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Closing connection'));
        ...
        it('should verify exact template literal in logger.debug for connecting', async ()=>{
            ...
            expect(logger.debug).toHaveBeenCalled();
            const connectCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Connecting to') && call[0]?.includes(executionId));
        ...
        it('should verify exact template literal in logger.debug for connected', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(`[WebSocket] Connected to execution ${executionId}`);
            }
        });
        it('should verify exact template literal in logger.debug for disconnected', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining(`Disconnected from execution ${executionId}`), expect.any(Object));
            }
        });
        it('should verify exact template literal in logger.debug for reconnecting', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalled();
                const reconnectCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Reconnecting in') && call[0]?.includes('attempt'));
        ...
        it('should verify exact template literal in logger.error for failed to create', async ()=>{
            ...
            expect(logger.error).toHaveBeenCalledWith(expect.stringContaining(`Failed to create connection for execution ${executionId}`), expect.any(Error));
            // Restore
            global.WebSocket = OriginalWebSocket;
        });
        ...
        it('should verify exact template literal in logger.debug for closing connection', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalled();
                const closeCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Closing connection') && call[0]?.includes(executionId));
                expect(closeCalls.length).toBeGreaterThanOrEqual(0);
            }
        });
        it('should verify exact template literal in logger.debug for skipping connection with status', async ()=>{
            ...
            expect(logger.debug).toHaveBeenCalled();
            const skipCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Skipping connection') && call[0]?.includes(executionId) && call[0]?.includes('completed'));
        ...
        it('should verify exact template literal in logger.debug for skipping reconnect with status', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalled();
                const skipCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Skipping reconnect') && call[0]?.includes(executionId));
                expect(skipCalls.length).toBeGreaterThanOrEqual(0);
            }
        });
        it('should verify exact template literal in logger.debug for skipping reconnect for pending', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.debug).toHaveBeenCalled();
                const skipCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Skipping reconnect for temporary execution ID'));
                expect(skipCalls.length).toBeGreaterThanOrEqual(0);
            }
        });
        ...
        it('should verify exact template literal in logger.error for connection error', async ()=>{
            ...
            if (wsInstances.length > 0) {
                ...
                expect(logger.error).toHaveBeenCalledWith(expect.stringContaining(`Connection error for execution ${executionId}:`), expect.any(Object));
            }
        });
        describe('Math operations coverage', ()=>{
            it('should verify Math.pow operation in reconnect delay calculation', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).toHaveBeenCalled();
                    const reconnectCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Reconnecting in'));
                    expect(reconnectCalls.length).toBeGreaterThan(0);
                }
            });
            ...
            it('should verify exact Math.pow calculation for first reconnect', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).toHaveBeenCalled();
                    const reconnectCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Reconnecting in') && call[0]?.includes('10000'));
                    expect(reconnectCalls.length).toBeGreaterThan(0);
                }
            });
        ...
        describe('wasClean and code comparison coverage', ()=>{
            it('should verify wasClean && code === 1000 condition', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).toHaveBeenCalled();
                    const cleanCloseCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Connection closed cleanly'));
        ...
        describe('reconnectAttempts comparison coverage', ()=>{
            ...
            it('should verify reason || "No reason provided" pattern', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).toHaveBeenCalled();
                    const disconnectCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Disconnected from execution'));
        ...
        describe('reconnectAttempts increment and comparison coverage', ()=>{
            it('should verify reconnectAttempts.current++ increment', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).toHaveBeenCalled();
                    const reconnectCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Reconnecting in') && call[0]?.includes('attempt 1/5'));
                    expect(reconnectCalls.length).toBeGreaterThan(0);
                }
            });
            ...
            it('should verify reconnectAttempts.current = 0 reset on executionId change', async ()=>{
                const { rerender } = renderHook(({ executionId })=>useWebSocket({
                        executionId,
                        executionStatus: 'running'
                    }), {
                    initialProps: {
        ...
        describe('currentStatus === comparisons coverage', ()=>{
            it('should verify currentStatus === "completed" comparison', async ()=>{
                ...
                expect(logger.debug).toHaveBeenCalled();
                const skipCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Skipping connection') && call[0]?.includes('completed'));
                expect(skipCalls.length).toBeGreaterThanOrEqual(0);
            });
            it('should verify currentStatus === "failed" comparison', async ()=>{
                ...
                // Verify exact comparison: currentStatus === 'completed' || currentStatus === 'failed'
                expect(logger.debug).toHaveBeenCalled();
                const skipCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Skipping connection') && call[0]?.includes('failed'));
                expect(skipCalls.length).toBeGreaterThanOrEqual(0);
            });
            it('should verify executionStatus === "completed" comparison in first useEffect', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).toHaveBeenCalled();
                    const closeCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Closing connection') && call[0]?.includes('completed'));
                    expect(closeCalls.length).toBeGreaterThanOrEqual(0);
                }
            });
            it('should verify executionStatus === "failed" comparison in first useEffect', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    // Verify exact comparison: executionStatus === 'completed' || executionStatus === 'failed'
                    expect(logger.debug).toHaveBeenCalled();
                    const closeCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Closing connection') && call[0]?.includes('failed'));
                    expect(closeCalls.length).toBeGreaterThanOrEqual(0);
                }
            });
            it('should verify currentStatus === "completed" || "failed" pattern in onclose', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    // Verify exact comparison: currentStatus === 'completed' || currentStatus === 'failed'
                    expect(logger.debug).toHaveBeenCalled();
                    const skipCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Skipping reconnect') && call[0]?.includes('completed'));
                    expect(skipCalls.length).toBeGreaterThanOrEqual(0);
                }
            });
        ...
        describe('onclose logical operators coverage', ()=>{
            it('should verify wasClean && code === 1000 pattern', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).toHaveBeenCalled();
                    const closeCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Connection closed cleanly'));
            ...
            it('should verify wasClean && code === 1000 pattern with false wasClean', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    const cleanCalls = allCalls.filter((call)=>call[0]?.includes('Connection closed cleanly'));
            ...
            it('should verify wasClean && code === 1000 pattern with different code', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    const allCalls = logger.debug.mock.calls;
                    const cleanCalls = allCalls.filter((call)=>call[0] === '[WebSocket] Connection closed cleanly, not reconnecting');
        ...
        describe('exact template literal coverage', ()=>{
            ...
            it('should verify exact template literal in logger.debug for connection', async ()=>{
                ...
                expect(logger.debug).toHaveBeenCalled();
                const connectCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('[WebSocket] Connecting to'));
                expect(connectCalls.length).toBeGreaterThan(0);
            });
            it('should verify exact template literal in logger.debug for connected', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    // Verify exact template literal: `[WebSocket] Connected to execution ${executionId}`
                    expect(logger.debug).toHaveBeenCalled();
                    const connectedCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Connected to execution'));
            ...
            it('should verify exact template literal in logger.error for connection error', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    // Verify exact template literal: `[WebSocket] Connection error for execution ${executionId}:`
                    expect(logger.error).toHaveBeenCalled();
                    const errorCalls = logger.error.mock.calls.filter((call)=>call[0]?.includes('Connection error for execution'));
                    expect(errorCalls.length).toBeGreaterThan(0);
                }
            });
            it('should verify exact template literal in logger.debug for disconnected', async ()=>{
            ...
            it('should verify exact template literal in logger.error for failed to create', async ()=>{
                ...
                expect(logger.error).toHaveBeenCalled();
                const errorCalls = logger.error.mock.calls.filter((call)=>call[0]?.includes('Failed to create connection for execution'));
                expect(errorCalls.length).toBeGreaterThan(0);
                global.WebSocket = OriginalWebSocket;
            });
        });
        ...
        describe('exact comparison operators coverage', ()=>{
            it('should verify code === 1000 exact comparison', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).toHaveBeenCalled();
                    const cleanCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Connection closed cleanly'));
        ...
        describe('exact assignment operations', ()=>{
            ...
            it('should verify reconnectAttempts.current++ increment', async ()=>{
                ...
                if (wsInstances.length > 0) {
                    ...
                    expect(logger.debug).toHaveBeenCalled();
                    const reconnectCalls = logger.debug.mock.calls.filter((call)=>call[0]?.includes('Reconnecting in') && call[0]?.includes('attempt 1/'));
                    expect(reconnectCalls.length).toBeGreaterThan(0);
                }
            });
