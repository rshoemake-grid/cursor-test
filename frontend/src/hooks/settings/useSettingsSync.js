 ...
 */ export function useSettingsSync(options) {
    const { isAuthenticated, token, providers, iterationLimit, defaultModel, settingsService, settingsLoaded, consoleAdapter } = options;
    /**
   ...
   */ const autoSaveSettings = useMemo(()=>async ()=>{
    ...
    useAutoSave({
        ...
        iterationLimit,
        defaultModel
    }, autoSaveSettings, 500, !!(isAuthenticated && token && settingsLoaded));
    /**
   * Manual sync handler
