...
function useWorkflowUpdates({
  ...
  const applyLocalChanges = useCallback((changes) => {
    ...
    if (changes.nodes_to_add && changes.nodes_to_add.length > 0) {
      ...
      logger.debug("Working nodes after addition:", workingNodes.map((n) => ({ id: n.id, type: n.type })));
      nodesTouched = true;
    }
    ...
    if (changes.nodes_to_delete && changes.nodes_to_delete.length > 0) {
      ...
      logger.debug("Current node IDs before deletion:", workingNodes.map((n) => n.id));
      workingNodes = workingNodes.filter((node) => !changes.nodes_to_delete.includes(node.id));
      ...
      logger.debug("Nodes after deletion:", workingNodes.map((n) => n.id));
      nodesTouched = true;
      edgesTouched = true;
    }
    ...
    if (changes.edges_to_add && changes.edges_to_add.length > 0) {
      ...
      logger.debug("Current edges:", workingEdges.map((e) => `${e.source} -> ${e.target}`));
