...
export function useWorkflowLoader({ workflowId, tabIsUnsaved, setNodes, setEdges, setLocalWorkflowId, setLocalWorkflowName, setLocalWorkflowDescription, setVariables, setSelectedNodeId, workflowNodeToNode, onWorkflowLoaded, isLoadingRef, lastLoadedWorkflowIdRef }) {
    useEffect(()=>{
        ...
        if (workflowId) {
            ...
            api.getWorkflow(workflowId).then((workflow)=>{
                ...
                logger.debug('Loaded nodes:', initializedNodes.map((n)=>({
                        id: n.id,
                        type: n.type,
                        position: n.position
                    })));
