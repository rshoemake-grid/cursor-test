...
import { hasOfficialItems, hasNoUserOwnedItems, ownsAllItems, ownsPartialItems } from '../utils/deletionValidation';
/**
 * Hook for deleting workflows/templates
 * 
 * @param options Configuration options
 * @returns Workflow deletion handler
 */ export function useWorkflowDeletion({ user, templates, workflowsOfWorkflows, activeTab, setTemplates, setWorkflowsOfWorkflows, setSelectedTemplateIds, showError = defaultShowError, showSuccess = defaultShowSuccess, showConfirm = defaultShowConfirm, api = defaultApi }) {
    const deleteSelectedWorkflows = useCallback(async (selectedTemplateIds)=>{
        ...
        if (ownsPartialItems(userOwnedTemplates.length, deletableTemplates.length)) {
            const confirmed = await showConfirm(`You can only delete ${userOwnedTemplates.length} of ${deletableTemplates.length} selected workflow(s). Delete only the ones you own?`, {
            ...
            const confirmed = await showConfirm(`Are you sure you want to delete ${userOwnedTemplates.length} selected workflow(s) from the marketplace?`, {
