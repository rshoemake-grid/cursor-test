    ...
    const applyLocalChanges = useCallback((changes)=>{
        ...
        if (changes.nodes_to_add && changes.nodes_to_add.length > 0) {
            ...
            setNodes((nds)=>{
                ...
                logger.debug('New nodes after addition:', newNodes.map((n)=>({
                        id: n.id,
                        type: n.type
                    })));
                return newNodes;
        ...
        if (changes.nodes_to_delete && changes.nodes_to_delete.length > 0) {
            ...
            setEdges((eds)=>eds.filter((edge)=>!changes.nodes_to_delete.includes(edge.source) && !changes.nodes_to_delete.includes(edge.target)));
            notifyModified();
        }
