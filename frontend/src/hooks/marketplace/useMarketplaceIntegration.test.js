...
describe('useMarketplaceIntegration', ()=>{
    ...
    describe('addAgentsToCanvas', ()=>{
        it('should add agents to canvas', ()=>{
            const { result } = renderHook(()=>useMarketplaceIntegration({
                    tabId: 'tab-1',
                    storage: null,
        ...
        it('should use current state values in draft update', async ()=>{
            ...
            const { result, rerender } = renderHook(({ localWorkflowId, localWorkflowName, tabIsUnsaved })=>useMarketplaceIntegration({
                    tabId: 'tab-1',
                    storage: null,
                    setNodes: mockSetNodes,
                    notifyModified: mockNotifyModified,
        ...
        it('should verify agent.name || agent.label || Agent Node - all branches', ()=>{
            ...
            const agentsWithoutNameOrLabel = [
                {
                    id: 'agent-3',
                    agent_config: {}
                }
        ...
        it('should verify currentDraft?.edges || [] fallback', async ()=>{
            ...
            mockTabDraftsRef.current['tab-1'] = undefined;
            let currentNodes = [];
            mockSetNodes.mockImplementation((updater)=>{
                if (typeof updater === 'function') {
                    currentNodes = updater(currentNodes);
    ...
    describe('useEffect event handling', ()=>{
        it('should verify targetTabId !== tabId check - different tab', ()=>{
            renderHook(()=>useMarketplaceIntegration({
                    tabId: 'tab-1',
                    storage: null,
                    setNodes: mockSetNodes,
                    notifyModified: mockNotifyModified,
        ...
        it('should verify catch block error handling', ()=>{
            ...
            expect(mockLoggerError).toHaveBeenCalledWith('Failed to process pending agents:', expect.any(Error));
            expect(mockStorage.removeItem).toHaveBeenCalledWith('pendingAgentsToAdd');
        });
        ...
        it('should verify cleanup removes event listener', ()=>{
            ...
            const { unmount } = renderHook(()=>useMarketplaceIntegration({
                    tabId: 'tab-1',
                    storage: null,
                    setNodes: mockSetNodes,
                    notifyModified: mockNotifyModified,
    ...
    describe('additional coverage for no-coverage mutants', ()=>{
        ...
        describe('mutation killers for addAgentsToCanvas', ()=>{
            ...
            it('should verify exact currentNodes.length > 0 comparison - length is 1', ()=>{
                ...
                const setNodesCall = mockSetNodes.mock.calls[0][0];
                const newNodes = typeof setNodesCall === 'function' ? setNodesCall([
                    existingNode
                ]) : setNodesCall;
            ...
            it('should verify exact currentDraft?.edges || [] fallback', ()=>{
                ...
                const { result: result1 } = renderHook(()=>useMarketplaceIntegration({
                        tabId: 'tab-1',
                        storage: null,
                        setNodes: mockSetNodes,
                        notifyModified: mockNotifyModified,
                ...
                act(()=>{
                    result1.current.addAgentsToCanvas([
                        {
                            id: 'agent-1',
                            name: 'Agent'
                        }
                ...
                mockTabDraftsRef.current = {};
                const { result: result2 } = renderHook(()=>useMarketplaceIntegration({
                        tabId: 'tab-1',
                        storage: null,
                        setNodes: mockSetNodes,
                        notifyModified: mockNotifyModified,
                ...
                act(()=>{
                    result2.current.addAgentsToCanvas([
                        {
                            id: 'agent-2',
                            name: 'Agent'
                        }
