 ...
 */ export function useTemplateUsage({ token, httpClient, apiBaseUrl, logger = defaultLogger }) {
    ...
    const useTemplate = useCallback(async (templateId)=>{
        try {
            ...
            const response = await httpClient.post(`${apiBaseUrl}/templates/${templateId}/use`, {}, headers);
            if (response.ok) {
