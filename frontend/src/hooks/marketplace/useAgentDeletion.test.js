...
describe('useAgentDeletion', ()=>{
    ...
    describe('deleteSelectedAgents', ()=>{
        it('should return early when no agents selected', async ()=>{
            const { result } = renderHook(()=>useAgentDeletion({
                    user: {
                        id: 'user-1',
                        username: 'testuser'
                    },
        ...
        it('should filter out official agents', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('Cannot delete 1 official agent(s)'));
        });
        ...
        it('should show partial delete confirmation when user owns some agents', async ()=>{
            ...
            expect(mockShowConfirm).toHaveBeenCalledWith(expect.stringContaining('You can only delete 1 of 2 selected agent(s)'), expect.objectContaining({
        ...
        it('should handle storage errors', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('Failed to delete agents'));
        });
    ...
    describe('deleteSelectedAgents', ()=>{
        ...
        it('should show error when user owns no agents and no author_id', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('were published before author tracking was added'));
        });
        it('should show error when user owns no agents but agents have author_id', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('You can only delete agents that you published'));
        });
        it('should show partial delete confirmation when user owns some agents', async ()=>{
            const agents = [
                {
        ...
        it('should show full delete confirmation when user owns all agents', async ()=>{
            ...
            expect(mockShowConfirm).toHaveBeenCalledWith(expect.stringContaining('Are you sure you want to delete 2 selected agent(s)'), expect.objectContaining({
                title: 'Delete Agents',
                confirmText: 'Delete',
                cancelText: 'Cancel',
                type: 'danger'
    ...
    describe('deleteSelectedAgents edge cases', ()=>{
        ...
        it('should handle author_id as null vs undefined', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('published before author tracking'));
        });
    ...
    describe('mutation killers for deleteSelectedAgents', ()=>{
        ...
        it('should verify officialAgents.length > 0 boundary (exactly 0)', async ()=>{
            ...
            expect(mockShowError).not.toHaveBeenCalledWith(expect.stringContaining('official agent'));
        });
        it('should verify officialAgents.length > 0 boundary (exactly 1)', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('official agent'));
        });
        ...
        it('should verify userOwnedAgents.length === 0 path with agentsWithAuthorId.length === 0 and officialAgents.length > 0', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith('Selected agents were published before author tracking was added or are official. Please republish them to enable deletion.');
        });
        it('should verify userOwnedAgents.length === 0 path with agentsWithAuthorId.length === 0 and officialAgents.length === 0', async ()=>{
            const agents = [
                {
            ...
            expect(mockShowError).toHaveBeenCalledWith('Selected agents were published before author tracking was added. Please republish them to enable deletion.');
        });
        it('should verify userOwnedAgents.length === 0 path with agentsWithAuthorId.length > 0 and officialAgents.length > 0', async ()=>{
            const agents = [
                {
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('You can only delete agents that you published (official agents cannot be deleted)'));
        });
        it('should verify userOwnedAgents.length === 0 path with agentsWithAuthorId.length > 0 and officialAgents.length === 0', async ()=>{
            const agents = [
                {
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('You can only delete agents that you published'));
            expect(mockShowError).not.toHaveBeenCalledWith(expect.stringContaining('official agents cannot be deleted'));
        });
        ...
        it('should verify userOwnedAgents.length < deletableAgents.length path', async ()=>{
            ...
            expect(mockShowConfirm).toHaveBeenCalledWith(expect.stringContaining('You can only delete 1 of 2 selected agent(s)'), expect.any(Object));
        });
        it('should verify userOwnedAgents.length === deletableAgents.length path', async ()=>{
            const agents = [
                {
            ...
            expect(mockShowConfirm).toHaveBeenCalledWith(expect.stringContaining('Are you sure you want to delete 1 selected agent(s)'), expect.any(Object));
        });
    ...
    describe('deleteSelectedRepositoryAgents', ()=>{
        it('should return early when no agents selected', async ()=>{
            const { result } = renderHook(()=>useRepositoryAgentDeletion({
                    storage: mockStorage,
        ...
        it('should handle storage errors', async ()=>{
            ...
            expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('Failed to delete repository agents'));
        });
    });
        ...
        describe('String conversion edge cases', ()=>{
            describe('String(author_id) conversion', ()=>{
                it('should verify exact String conversion - author_id is number', ()=>{
                    ...
                    act(()=>{
                        result.current.deleteSelectedAgents(new Set([
                            'agent-1'
                        ]));
                    });
        ...
        describe('Boundary conditions', ()=>{
            describe('officialAgents.length === 0 vs > 0', ()=>{
                ...
                it('should verify exact length check - officialAgents.length > 0', ()=>{
                    ...
                    expect(mockShowError).toHaveBeenCalledWith(expect.stringContaining('Cannot delete'));
                });
            });
            describe('userOwnedAgents.length === deletableAgents.length', ()=>{
                it('should verify exact length comparison - equal', ()=>{
                    ...
                    // Should show full delete confirmation (not partial)
                    expect(mockShowConfirm).toHaveBeenCalledWith(expect.stringContaining('Are you sure'), expect.any(Object));
                });
                it('should verify exact length comparison - less than', ()=>{
                    ...
                    // Should show partial delete confirmation
                    expect(mockShowConfirm).toHaveBeenCalledWith(expect.stringContaining('only delete 1 of 2'), expect.any(Object));
                });
            });
        });
    ...
    describe('no-coverage paths for deleteSelectedAgents', ()=>{
        describe('deleteSelectedAgents - catch blocks', ()=>{
            it('should handle storage.getItem throwing error', async ()=>{
                ...
                expect(showError).toHaveBeenCalledWith(expect.stringContaining('Failed to delete agents'));
            });
    ...
    describe('no-coverage paths for deleteSelectedRepositoryAgents', ()=>{
        describe('deleteSelectedRepositoryAgents - catch blocks', ()=>{
            it('should handle storage operations throwing error', async ()=>{
                ...
                expect(showError).toHaveBeenCalledWith(expect.stringContaining('Failed to delete repository agents'));
            });
        });
    });
    describe('branch coverage for deleteSelectedRepositoryAgents', ()=>{
        describe('deleteSelectedRepositoryAgents - storage branches', ()=>{
            ...
            it('should successfully delete repository agents and call onRefresh', async ()=>{
                ...
                expect(mockStorage.setItem).toHaveBeenCalledWith(STORAGE_KEYS.REPOSITORY_AGENTS, expect.stringContaining('agent-2'));
                expect(mockSetRepositoryAgents).toHaveBeenCalled();
