...
describe('useMarketplaceActions', ()=>{
    ...
    describe('handleUseAgents', ()=>{
        it('should add agents to workflow from agents tab', ()=>{
            ...
            expect(mockStorage.setItem).toHaveBeenCalledWith(PENDING_AGENTS_STORAGE_KEY, expect.stringContaining('"tabId":"tab-123"'));
        ...
        it('should add repository agents to workflow from repository agents tab', ()=>{
            ...
            const setItemCall = mockStorage.setItem.mock.calls.find((call)=>call[0] === PENDING_AGENTS_STORAGE_KEY);
            expect(setItemCall).toBeDefined();
            const storedData = JSON.parse(setItemCall[1]);
            expect(storedData.agents).toEqual([
                mockRepositoryAgents[0]
            ]);
        ...
        it('should dispatch custom event when adding agents', ()=>{
            ...
            expect(event.detail.agents).toEqual([
                mockAgents[0]
            ]);
    ...
    describe('handleUseTools', ()=>{
        it('should add selected tools to workflow', ()=>{
            ...
            expect(mockStorage.getItem).toHaveBeenCalledWith(STORAGE_KEYS.ACTIVE_TAB);
            expect(mockStorage.setItem).toHaveBeenCalledWith(PENDING_TOOLS_STORAGE_KEY, expect.stringContaining('"tabId":"tab-123"'));
        ...
        it('should dispatch ADD_TOOLS_TO_WORKFLOW event', ()=>{
            ...
            expect(dispatchEventSpy).toHaveBeenCalled();
            const event = dispatchEventSpy.mock.calls.find((call)=>call[0].type === MARKETPLACE_EVENTS.ADD_TOOLS_TO_WORKFLOW);
            expect(event).toBeDefined();
            const eventDetail = event[0].detail;
            expect(eventDetail.tabId).toBe('tab-123');
            expect(eventDetail.tools).toEqual([
                mockTools[0]
            ]);
            dispatchEventSpy.mockRestore();
        });
    ...
    describe('handleDeleteRepositoryAgents', ()=>{
        it('should delete selected repository agents', async ()=>{
            ...
            expect(mockDeleteSelectedRepositoryAgents).toHaveBeenCalledWith(mockRepositoryAgentSelection.selectedIds, mockFetchRepositoryAgents);
        });
        it('should handle empty selection', async ()=>{
            const { result } = renderHook(()=>useMarketplaceActions(defaultOptions));
            // Ensure hook is initialized
            ...
            expect(mockDeleteSelectedRepositoryAgents).toHaveBeenCalledWith(new Set(), mockFetchRepositoryAgents);
        });
    });
});
