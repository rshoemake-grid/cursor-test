...
describe('useMarketplaceData - Mutation Killers', ()=>{
    ...
    describe('fetchTemplates - exact truthy checks', ()=>{
        ...
        it('should verify exact truthy check - category is empty string (should not append)', async ()=>{
            ...
            expect(mockHttpClient.get).toHaveBeenCalledWith(expect.not.stringContaining('category='));
        });
        ...
        it('should verify exact truthy check - searchQuery is empty string (should not append)', async ()=>{
            ...
            expect(mockHttpClient.get).toHaveBeenCalledWith(expect.not.stringContaining('search='));
        });
    });
    describe('fetchAgents - exact AND operator and truthy checks', ()=>{
        ...
        it('should verify exact logical OR - user.username || user.email || null', async ()=>{
            ...
            mockGetLocalStorageItem.mockReturnValue(agents);
            // Test username path
            const { result: result1 } = renderHook(()=>useMarketplaceData({
                    storage: mockStorage,
                    httpClient: mockHttpClient,
                    apiBaseUrl: 'http://api.test',
                    category: '',
            ...
            const { result: result2 } = renderHook(()=>useMarketplaceData({
                    storage: mockStorage,
                    httpClient: mockHttpClient,
                    apiBaseUrl: 'http://api.test',
                    category: '',
            ...
            mockStorage.setItem.mockClear();
            const { result: result3 } = renderHook(()=>useMarketplaceData({
                    storage: mockStorage,
                    httpClient: mockHttpClient,
                    apiBaseUrl: 'http://api.test',
                    category: '',
