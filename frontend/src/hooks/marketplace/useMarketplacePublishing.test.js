...
describe('useMarketplacePublishing', ()=>{
    ...
    describe('openPublishModal', ()=>{
        it('should show error when activeTab is undefined', ()=>{
            const { result } = renderHook(()=>useMarketplacePublishing({
                    activeTab: undefined,
                    token: 'test-token',
    ...
    describe('handlePublish', ()=>{
        ...
        it('should publish successfully', async ()=>{
            ...
            expect(mockHttpClient.post).toHaveBeenCalledWith('http://api.test/workflows/workflow-1/publish', {
                category: 'automation',
                tags: [
                    'tag1',
                    'tag2'
        ...
        it('should handle tags.split and filter', async ()=>{
            ...
            expect(mockHttpClient.post).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                tags: [
                    'tag1',
                    'tag2',
                    'tag3'
        ...
        it('should verify token ? Authorization : {} check - token exists', async ()=>{
            ...
            expect(mockHttpClient.post).toHaveBeenCalledWith(expect.any(String), expect.any(Object), expect.objectContaining({
                Authorization: 'Bearer test-token'
            }));
        });
        it('should verify token ? Authorization : {} check - token is null', async ()=>{
            ...
            expect(mockHttpClient.post).toHaveBeenCalledWith(expect.any(String), expect.any(Object), expect.not.objectContaining({
        ...
        it('should verify !activeTab || !activeTab.workflowId checks both conditions', async ()=>{
            ...
            const { result: result1 } = renderHook(()=>useMarketplacePublishing({
                    activeTab: undefined,
                    token: 'test-token',
                    httpClient: mockHttpClient,
                    apiBaseUrl: 'http://api.test'
            ...
            const { result: result2 } = renderHook(()=>useMarketplacePublishing({
                    activeTab: {
                        id: 'tab-1',
                        workflowId: null,
                        name: 'Test'
