...
import { hasOfficialItems, hasNoUserOwnedItems, ownsAllItems, ownsPartialItems, getItemsWithAuthorIdCount } from '../utils/deletionValidation';
/**
 ...
 */ export function useAgentDeletion({ user, storage, agents, setAgents, setSelectedAgentIds, showError = defaultShowError, showSuccess = defaultShowSuccess, showConfirm = defaultShowConfirm, logger = defaultLogger }) {
    const deleteSelectedAgents = useCallback(async (selectedAgentIds)=>{
        ...
        if (ownsPartialItems(userOwnedAgents.length, deletableAgents.length)) {
            const confirmed = await showConfirm(`You can only delete ${userOwnedAgents.length} of ${deletableAgents.length} selected agent(s). Delete only the ones you own?`, {
            ...
            const confirmed = await showConfirm(`Are you sure you want to delete ${userOwnedAgents.length} selected agent(s) from the marketplace?`, {
                title: 'Delete Agents',
                confirmText: 'Delete',
                cancelText: 'Cancel',
                type: 'danger'
        ...
        const result = deleteAgentsFromStorage(storage, STORAGE_KEYS.PUBLISHED_AGENTS, agentIdsToDelete, {
            showError,
            showSuccess,
            errorPrefix: 'agents'
        });
 ...
 */ export function useRepositoryAgentDeletion({ storage, setRepositoryAgents, setSelectedRepositoryAgentIds, showError = defaultShowError, showSuccess = defaultShowSuccess, showConfirm = defaultShowConfirm }) {
    const deleteSelectedRepositoryAgents = useCallback(async (selectedRepositoryAgentIds, onRefresh)=>{
        ...
        if (!confirmed) return;
        // Delete from storage using shared service - SRP + DRY
        const result = deleteAgentsFromStorage(storage, STORAGE_KEYS.REPOSITORY_AGENTS, selectedRepositoryAgentIds, {
            showError,
            showSuccess,
