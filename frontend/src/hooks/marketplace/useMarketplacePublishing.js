 ...
 */ export function useMarketplacePublishing({ activeTab, token, httpClient, apiBaseUrl, showError = defaultShowError, showSuccess = defaultShowSuccess }) {
    ...
    const handlePublishFormChange = useCallback((field, value)=>{
        publishFormHook.updateField(field, value);
    }, [
        publishFormHook
    ]);
    const handlePublish = useCallback(async (event)=>{
        ...
        try {
            ...
            const response = await httpClient.post(`${apiBaseUrl}/workflows/${activeTab.workflowId}/publish`, {
