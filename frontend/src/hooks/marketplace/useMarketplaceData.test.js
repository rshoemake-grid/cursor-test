...
describe('useMarketplaceData', ()=>{
    ...
    describe('fetchTemplates', ()=>{
        it('should fetch templates successfully', async ()=>{
            ...
            expect(mockHttpClient.get).toHaveBeenCalledWith(expect.stringContaining('http://api.test/templates'));
        ...
        it('should include category in params when provided', async ()=>{
            ...
            expect(mockHttpClient.get).toHaveBeenCalledWith(expect.stringContaining('category=automation'));
        });
        it('should include search query in params when provided', async ()=>{
            ...
            expect(mockHttpClient.get).toHaveBeenCalledWith(expect.stringContaining('search=test+query'));
        });
        it('should include sort_by in params', async ()=>{
            ...
            expect(mockHttpClient.get).toHaveBeenCalledWith(expect.stringContaining('sort_by=recent'));
        });
    ...
    describe('setters', ()=>{
        ...
        it('should allow setting repository agents', ()=>{
            ...
            act(()=>{
                result.current.setRepositoryAgents([
                    mockAgent
                ]);
            });
            expect(result.current.repositoryAgents).toHaveLength(1);
        ...
        it('should allow setting workflows of workflows', ()=>{
            ...
            act(()=>{
                result.current.setWorkflowsOfWorkflows([
                    mockTemplate
                ]);
            });
            expect(result.current.workflowsOfWorkflows).toHaveLength(1);
    ...
    describe('fetchWorkflowsOfWorkflows edge cases', ()=>{
        ...
        it('should verify hasWorkflowReference checks node.workflow_id', async ()=>{
            const templateWithWorkflowId = {
                ...mockTemplate,
                id: 'template-wf'
            };
            mockHttpClient.get.mockResolvedValue({
                json: async ()=>[
                        templateWithWorkflowId
                    ]
            });
            mockHttpClient.post.mockResolvedValue({
                ok: true,
        ...
        it('should handle error when checking individual workflow', async ()=>{
            ...
            expect(mockLoggerError).toHaveBeenCalledWith('Failed to check workflow template-1:', expect.any(Error));
    ...
    describe('fetchAgents edge cases', ()=>{
        ...
        it('should verify updated && storage check with null storage', async ()=>{
            ...
            expect(mockLoggerDebug).not.toHaveBeenCalledWith(expect.stringContaining('Updated agents with author info'), expect.any(String));
        });
    ...
    describe('useEffect activeTab routing', ()=>{
        ...
        it('should verify exact fallback value savedAgents ? JSON.parse(savedAgents) : []', ()=>{
            ...
            const savedAgents1 = JSON.stringify([
                {
                    id: 'agent-1'
                }
            ]);
            const agentsData1 = savedAgents1 ? JSON.parse(savedAgents1) : [];
            expect(agentsData1).toEqual([
                {
                    id: 'agent-1'
                }
            ]);
    ...
    describe('additional coverage for no-coverage mutants', ()=>{
        describe('fetchWorkflowsOfWorkflows - inner catch block', ()=>{
            it('should handle error when checking individual workflow', async ()=>{
                ...
                expect(mockLoggerError).toHaveBeenCalledWith(expect.stringContaining('Failed to check workflow'), expect.any(Error));
            });
    ...
    describe('mutation killers - string operations and logical operators', ()=>{
        describe('fetchAgents - string operations', ()=>{
            ...
            it('should verify exact includes() call on description.toLowerCase()', async ()=>{
                ...
                const filtered = result.current.agents.filter((a)=>a.description.toLowerCase().includes('description'));
                expect(filtered.length).toBeGreaterThan(0);
            });
            it('should verify exact includes() call on tags with some()', async ()=>{
                ...
                const filtered = result.current.agents.filter((a)=>a.tags.some((tag)=>tag.toLowerCase().includes('test-tag')));
                expect(filtered.length).toBeGreaterThan(0);
