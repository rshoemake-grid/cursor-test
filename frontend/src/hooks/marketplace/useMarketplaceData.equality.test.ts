...
describe('useMarketplaceData - Equality Operators', () => {
    ...
    it('should NOT match "Repository" (case sensitive)', async () => {
      ...
      const { result } = renderHook(() =>
        useMarketplaceData({
          ...
          sortBy: 'popular',
          user: null,
          activeTab: 'Repository' as any, // Case mismatch - should not match 'repository'
          repositorySubTab: 'workflows',
        })
      )
  ...
  describe('RepositorySubTab equality - repositorySubTab === "workflows"', () => {
    ...
    it('should NOT match "workflow" (singular)', async () => {
      // Mock storage to return repository agents when called with REPOSITORY_AGENTS key
      ...
      // Should fetch repository agents (not exact match) - fallback when sub-tab is invalid
