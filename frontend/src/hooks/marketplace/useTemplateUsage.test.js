...
describe('useTemplateUsage', ()=>{
    ...
    describe('useTemplate', ()=>{
        it('should use template successfully with token', async ()=>{
            ...
            const { result } = renderHook(()=>useTemplateUsage({
                    token: 'token-123',
            ...
            expect(mockHttpClient.post).toHaveBeenCalledWith('http://api.test/templates/template-1/use', {}, expect.objectContaining({
    ...
    describe('mutation killers for useTemplate', ()=>{
        ...
        it('should verify exact logger.debug message content', async ()=>{
            ...
            expect(mockLoggerDebug).toHaveBeenCalledWith('Created workflow from template:', {
                id: 'workflow-123'
            });
        });
        it('should verify exact logger.error message content', async ()=>{
            ...
            expect(mockLoggerError).toHaveBeenCalledWith('Failed to use template:', 'Error message');
        });
    });
    describe('no-coverage paths for useTemplate', ()=>{
        describe('useTemplate - catch block', ()=>{
            it('should handle httpClient.post throwing error', async ()=>{
                ...
                expect(logger.error).toHaveBeenCalledWith('Failed to use template:', expect.any(Error));
            });
    ...
    describe('branch coverage for useTemplate', ()=>{
        describe('useTemplate - response.ok branches', ()=>{
            it('should navigate when response.ok is true', async ()=>{
                ...
                expect(logger.debug).toHaveBeenCalledWith('Created workflow from template:', {
                    id: 'workflow-123'
                });
            });
