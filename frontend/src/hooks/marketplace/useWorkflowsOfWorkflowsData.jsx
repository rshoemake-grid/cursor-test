      ...
      try {
        ...
        if (workflowResponse.ok) {
          ...
          if (workflowDetail.nodes && Array.isArray(workflowDetail.nodes)) {
            ...
            const workflowDescription = descResult !== null && descResult !== void 0 ? descResult : "";
            const descLower = workflowDescription.toLowerCase();
            const isWorkflowOfWorkflows = descLower.includes("workflow of workflows") || descLower.includes("composite workflow") || workflowDescription.includes("nested workflow") || workflow.tags && workflow.tags.some(
              (tag) => tag.toLowerCase().includes("workflow-of-workflows") || tag.toLowerCase().includes("composite") || tag.toLowerCase().includes("nested")
            );
