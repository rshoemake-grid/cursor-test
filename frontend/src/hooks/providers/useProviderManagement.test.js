...
describe('useProviderManagement', ()=>{
    ...
    const renderHookWithDefaults = (overrides = {})=>{
        return renderHook(()=>useProviderManagement({
                service: mockService,
                providers,
    ...
    describe('updateProvider', ()=>{
        it('should update provider and save', async ()=>{
            ...
            const updatedProviders = providers.map((p)=>p.id === 'provider-1' ? {
                    ...p,
                    enabled: false
                } : p);
        ...
        it('should not update non-existent provider', async ()=>{
            const { result } = renderHookWithDefaults();
            await act(async ()=>{
                await result.current.updateProvider('non-existent', {
                    enabled: false
                });
            });
    ...
    describe('testProvider', ()=>{
        ...
        it('should set testingProvider during test', async ()=>{
            ...
            mockService.testProvider.mockImplementation(()=>testPromise);
            const { result } = renderHookWithDefaults();
            // Start the test
    ...
    describe('addCustomModel', ()=>{
        ...
        it('should handle provider without models array', async ()=>{
            ...
            expect(updatedProvider.models).toEqual([
                'new-model'
            ]);
        });
    });
    describe('state management', ()=>{
        ...
        it('should maintain test results across multiple tests', async ()=>{
            mockService.testProvider.mockResolvedValueOnce({
                status: 'success',
                message: 'OK 1'
