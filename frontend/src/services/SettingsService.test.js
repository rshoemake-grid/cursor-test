...
describe('SettingsService', ()=>{
    ...
    describe('saveSettings', ()=>{
        ...
        it('should save to storage when storage is available', async ()=>{
            ...
            expect(mockStorage.setItem).toHaveBeenCalledWith('llm_settings', JSON.stringify(mockSettings));
        });
        ...
        it('should save to backend when token is provided', async ()=>{
            ...
            expect(mockHttpClient.post).toHaveBeenCalledWith('/api/settings/llm', mockSettings, expect.objectContaining({
                'Authorization': 'Bearer test-token'
            }));
        });
        ...
        it('should throw error when backend returns non-ok response', async ()=>{
            ...
            service = new SettingsService(mockHttpClient, mockStorage, apiBaseUrl);
            await expect(service.saveSettings(mockSettings, 'test-token')).rejects.toThrow('Failed to save settings: 500 Internal Server Error');
        });
    ...
    describe('testProvider', ()=>{
        ...
        it('should return success when provider test succeeds', async ()=>{
            ...
            expect(mockHttpClient.post).toHaveBeenCalledWith('/api/settings/llm/test', {
                type: 'openai',
