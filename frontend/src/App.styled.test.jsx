...
import { LayoutRoot, Title, NavTabButton } from "./App.styled";
...
describe("App.styled", () => {
  it("renders layout primitives", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Agentic Workflow Builder/i }),
    ).toBeInTheDocument();
  });

  it("applies active styles when $active is true", () => {
    const { rerender } = render(
      <NavTabButton type="button" $active={false}>
        Tab
      </NavTabButton>,
    );
    ...
    rerender(
      <NavTabButton type="button" $active>
        Tab
      </NavTabButton>,
    );
