import { EXECUTION_STATUSES, isValidExecutionStatus as isValidExecutionStatusConstant } from '../constants/stringLiterals';
/**
 ...
 */ export const getExecutionStatusColor = (status)=>{
    ...
    return color !== null && color !== undefined && color !== '' ? color : statusMap[EXECUTION_STATUSES.PAUSED];
};
/**
 * Get Tailwind CSS classes for execution status badge (light theme variant)
 * Uses constants to kill StringLiteral mutations
 */ export const getExecutionStatusColorLight = (status)=>{
    const statusMap = {
