 ...
 */ export function convertEdgesToWorkflowFormat(edges) {
 ...
 */ export function convertNodesToWorkflowFormat(nodes) {
    return nodes.map((node)=>{
        // Use coalesceStringChain to kill ConditionalExpression mutations
        const nodeName = coalesceStringChain(node.id, typeof node.data.name === 'string' && node.data.name !== '' ? node.data.name : null, typeof node.data.label === 'string' && node.data.label !== '' ? node.data.label : null);
        return {
            id: node.id,
            type: node.type,
            name: nodeName,
 ...
 */ export function initializeReactFlowNodes(nodes) {
    return nodes.map((node)=>({
            ...node,
            draggable: true,
            selected: false,
 ...
 */ export function formatEdgesForReactFlow(edges) {
    return edges.map((edge)=>{
 ...
 */ export function normalizeNodeForStorage(node) {
    return {
        ...node,
        data: {
            ...node.data,
 ...
 */ export function workflowNodeToReactFlowNode(wfNode, nodeExecutionStates) {
    ...
    return {
        id: wfNode.id,
        type: wfNode.type,
        position: coalesceObject(wfNode.position, {
            x: 0,
            y: 0
        }),
        draggable: true,
        ...
        data: {
            ...
            description: coalesceStringChain('', data.description, wfNode.description),
