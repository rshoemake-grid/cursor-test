...
// Use real useMarketplaceTabs hook (simple, no dependencies)
...
describe('MarketplacePage', ()=>{
    ...
    it('should render marketplace page', ()=>{
        renderWithRouter(/*#__PURE__*/ _jsx(MarketplacePage, {}));
    ...
    it('should handle sort by selection', async ()=>{
        ...
        await waitForWithTimeout(()=>{
            ...
            if (sortSelects.length > 0) {
                fireEvent.change(sortSelects[sortSelects.length - 1], {
                    target: {
                        value: 'newest'
                    }
                });
    ...
    describe('Dependency Injection', ()=>{
        it('should use injected HTTP client', async ()=>{
            ...
            await waitForWithTimeout(()=>{
                expect(mockUseMarketplaceData).toHaveBeenCalledWith(expect.objectContaining({
                    httpClient: mockHttpClient
                }));
            });
        });
    ...
    describe('Tab switching and workflows-of-workflows', ()=>{
        ...
        it('should fetch workflows-of-workflows with workflow references', async ()=>{
            ...
            const { rerender } = renderWithRouter(/*#__PURE__*/ _jsx(MarketplacePage, {}));
            ...
            rerender(/*#__PURE__*/ _jsx(MarketplacePage, {}));
    ...
    describe('Agent operations', ()=>{
        ...
        it('should filter agents by category', async ()=>{
            ...
            await waitForWithTimeout(()=>{
                ...
                const categorySelect = categorySelects.find((select)=>select.options[0]?.textContent === 'All Categories');
                if (categorySelect) {
    ...
    describe('UI interactions', ()=>{
        ...
        it('should handle repository agents sub-tab', async ()=>{
            ...
            await waitForWithTimeout(()=>{
                ...
                const agentsSubTab = buttons.find((btn)=>btn.textContent?.includes('Agents') && btn.closest('div')?.querySelector('button')?.textContent?.includes('Workflows'));
                if (agentsSubTab) {
        ...
        it('should display empty state for workflows', async ()=>{
            ...
            // Mock useMarketplaceData to return empty templates for repository workflows sub-tab
            mockUseMarketplaceData.mockReturnValue({
                templates: [],
                workflowsOfWorkflows: [],
                agents: [],
            ...
            // Click repository tab to switch to repository workflows sub-tab
            // Real hook will update state, React will re-render automatically
            await act(async ()=>{
            ...
            // Wait for repository sub-tabs to appear (confirms repository tab is active)
            // The sub-tabs only render when isRepositoryTab is true
            await waitForWithTimeout(()=>{
                // Find the repository sub-tab buttons - they appear in a separate div
                const subTabButtons = screen.getAllByRole('button').filter((btn)=>btn.textContent === 'Workflows' || btn.textContent === 'Agents');
                // Should have 2 sub-tab buttons (Workflows and Agents)
                expect(subTabButtons.length).toBeGreaterThanOrEqual(2);
            }, 3000);
            // Now wait for the empty state message for workflows
            // This confirms the workflows sub-tab is active (default when repository tab is clicked)
            await waitForWithTimeout(()=>{
