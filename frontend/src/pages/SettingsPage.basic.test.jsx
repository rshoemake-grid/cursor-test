...
describe('SettingsPage - Basic', ()=>{
    ...
    it('should show add provider form when add button is clicked', async ()=>{
        ...
        await waitForWithTimeout(()=>{
            const buttons = screen.getAllByRole('button');
            const addButton = buttons.find((btn)=>btn.textContent?.includes('Add') && !btn.textContent?.includes('Sync'));
            if (addButton) {
    ...
    it('should load providers from localStorage', async ()=>{
        ...
        localStorage.setItem(STORAGE_KEYS.LLM_SETTINGS, JSON.stringify({
