 ...
 * )
 * ```
 */ export async function waitForStateValue(getValue, expectedValue, timeout) {
 ...
 * )
 * ```
 */ export async function waitForArrayLength(getArray, minLength = 1, timeout) {
    const defaultTimeout = isRunningUnderStryker() ? 60000 : 30000;
    const actualTimeout = timeout ?? defaultTimeout;
    await waitFor(()=>{
 ...
 * )
 * ```
 */ export async function waitForCondition(condition, timeout) {
    const defaultTimeout = isRunningUnderStryker() ? 60000 : 30000;
    const actualTimeout = timeout ?? defaultTimeout;
    await waitFor(()=>{
