...
- **504 / proxy errors in the browser:** the CRA dev server could not reach the API on `PROXY_TARGET`. Start the Java API from `backend-java`: `./gradlew bootRun` (default port 8000), or point `PROXY_TARGET` at wherever the API listens.
